...
function isTransientCloudSaveError(error){return !error?.status||[429,500,502,503,504].includes(Number(error.status))||error?.name==='AbortError'}
async function sendCloudSavePayload(payload){
  let lastError=null;
  for(let attempt=0;attempt<3;attempt++){
    try{
      const r=await fetchWithTimeout('/api/save',{method:'POST',headers:{'Content-Type':'application/json'},body:payload},30000);
      return await readApiPayload(r);
    }catch(error){
      lastError=error;
      if(!isTransientCloudSaveError(error)||attempt===2) throw error;
      await waitMs([700,1600,3200][attempt]+Math.floor(Math.random()*250));
    }
  }
...
async function cloudSaveNow(d=CLOUD_DATA){
  ...
  CLOUD_SAVE_IN_FLIGHT=(async()=>{
    ...
    while(CLOUD_SAVE_PENDING&&CLOUD_SESSION){
      ...
      const payload=JSON.stringify({data:CLOUD_DATA});
      response=await sendCloudSavePayload(payload);
    }
    return response;
  })();
  try{return await CLOUD_SAVE_IN_FLIGHT}finally{
    CLOUD_SAVE_IN_FLIGHT=null;
    if(CLOUD_SAVE_PENDING&&CLOUD_SESSION) setTimeout(()=>cloudSaveNow(CLOUD_DATA).catch(handleCloudSaveFailure),250);
  }
}
function handleCloudSaveFailure(e){
  ...
  alert('La sauvegarde sécurisée a échoué : '+detail);
}
...
function globalPrintHeaderHTML(company){
  ...
  const logo=p.logo?`<img src="${esc(p.logo)}" alt="Logo ${esc(p.name)}">`:esc(companyInitials(p.name));
  return `<header class="g3ph" style="--company-primary:${p.primary};--company-accent:${p.accent}">
    <div class="g3phTop">
      <div class="g3phBrand"><div class="g3phLogo">${logo}</div><b title="${esc(p.name)}">${esc(p.name)}</b></div>
      <div class="g3phCol"><div title="${esc(p.name)}"><strong>Raison sociale :</strong> <span>${esc(p.name)}</span></div><div title="${safe(p.rccm)}"><strong>RCCM :</strong> <span>${safe(p.rccm)}</span></div></div>
      <div class="g3phCol"><div title="${safe(p.legalForm)}"><strong>Forme juridique :</strong> <span>${safe(p.legalForm)}</span></div><div title="${safe(p.phone)}"><strong>Téléphone :</strong> <span>${safe(p.phone)}</span></div></div>
      <div class="g3phCol"><div title="${safe(p.taxAccount)}"><strong>Compte contribuable :</strong> <span>${safe(p.taxAccount)}</span></div><div title="${safe(p.email)}"><strong>E-mail :</strong> <span>${safe(p.email)}</span></div></div>
    </div>
    <div class="g3phMeta">
      ...
      <div class="g3phBand"><div><strong>Adresse :</strong> ${safe(p.address)}</div><div><strong>Année de gestion :</strong> ${year}</div><div><strong>Exercice actif :</strong> ${safe(exercise)}</div></div>
    </div>
  </header>`;
}
...
function bilanSalesRows(items,sales){
  ...
  (sales||[]).forEach(s=>{
    ...
    const r=map[key]; r.count+=1; r.clientsServed+=saleClientsServedValue(s); r.qty+=Number(s.qty||0); r.total+=Number(s.total||0); r.serviceFee+=Number(s.serviceFee||0); r.charges+=Number(s.charges||0); r.profit+=Number(s.profit||0);
  });
  return Object.values(map).sort((a,b)=>a.cat.localeCompare(b.cat,'fr')||a.name.localeCompare(b.name,'fr'));
}
...
function showBilan(){
  ...
  shell(`<section class="section printable active bilanDetailPage"><div class="bilanToolbar no-print"><button onclick="show('rapports')">← Retour aux rapports</button><label>Mois<select id="bilanMonth">${monthOptions}</select></label><label>Année<input id="bilanYear" type="number" min="2000" max="2200" value="${getManageYear()}"></label><button onclick="applyBilanPeriod()">Actualiser</button><button onclick="window.print()">Imprimer / PDF</button><button onclick="exportBilanCsv()">Exporter Excel (CSV)</button></div>${warning}${buildBilanOfficialReport(company,items,sales,obligations,generatedAt)}</section>`,'rapports',{noGlobalPrintHeader:true,noGlobalFooter:true});
}
...
function openPublicCart(companyId){
  ...
  const html=`<div class="marketPayModalBackdrop publicOrderBackdrop" id="publicCartModal"><div class="marketPayModal publicCartBox publicOrderForm"><button class="marketPayClose" onclick="document.getElementById('publicCartModal')?.remove()">×</button>
    ...
    <div class="publicCheckoutGrid">
      <section class="publicOrderSection"><h3>2. Récapitulatif</h3><div class="publicPricingSummary"><div><span>Sous-total des articles</span><b>${money(pricing.subtotal)}</b></div><div><span>Frais de livraison (${deliveryRateLabel(pricing.deliveryRate)})</span><b>${money(pricing.deliveryFee)}</b></div><div class="grandTotal"><span>Total à payer</span><b>${money(pricing.total)}</b></div></div><details class="deliveryFeeDetails"><summary>Voir le barème des frais de livraison</summary><p>5 à 4 999 F : 10 % · 5 000 à 24 999 F : 5 % · 25 000 à 99 999 F : 2 % · 100 000 F et plus : 1,5 %.</p></details></section>
      <section class="publicOrderSection"><h3>3. Mode de paiement</h3><div class="checkoutTimingChoices"><button type="button" class="active" data-checkout-timing="delivery" onclick="selectPublicCheckoutMode('${companyId}','delivery')"><span>🚚</span><b>Payer à la livraison</b><small>Réglez lorsque votre commande vous est remise.</small></button><button type="button" data-checkout-timing="now" onclick="selectPublicCheckoutMode('${companyId}','now')"><span>⚡</span><b>Payer maintenant</b><small>Utilisez Wave ou USDT TRC20.</small></button></div><div id="publicCartPayBox" class="publicPaymentChoiceBox checkoutPaymentPanel"><div class="deliveryPaymentNotice"><span>🚚</span><div><b>Paiement à la livraison</b><p>Aucun identifiant de transaction n’est requis maintenant.</p></div></div><button class="paidBtn deliveryValidateBtn" onclick="finalizePublicCartPayment('${companyId}','PAIEMENT À LA LIVRAISON')" ${cart.length?'':'disabled'}>Valider ma commande</button></div></section>
    </div>
...
function renderPublicShop(slug){
  ...
  app.innerHTML=`<div class="publicShop officialStore">
    ...
    <main>
      <section class="officialCategoriesSection">
        <div class="officialSectionHeading"><div><span>NOS UNIVERS</span><h2>Catégories populaires</h2></div><button type="button" onclick="selectOfficialShopCategory('')">Voir toutes les catégories ${officialShopIcon('arrow')}</button></div>
        <div class="officialCategoriesGrid">${popularCards}</div>
      </section>
      ...
      <section class="officialTrustStats"><div><span>${officialShopIcon('user')}</span><b>+5000</b><small>Clients satisfaits</small></div><div><span>${officialShopIcon('bag')}</span><b>+10000</b><small>Produits vendus</small></div><div><span>${officialShopIcon('support')}</span><b>24/7</b><small>Service disponible</small></div><div><span>${officialShopIcon('shield')}</span><b>Garantie</b><small>Satisfait ou remboursé</small></div></section>
    </main>
