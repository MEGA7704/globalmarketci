...
const FREE_PLAN_DAYS=10;
const STANDARD_PLAN_DAYS=30;
const BUSINESS_PLAN_DAYS=365;
const STANDARD_PLAN_AMOUNT=4800;
const BUSINESS_PLAN_AMOUNT=46100;
const GLOBAL_MARKET_PLANS={
  FREE:{code:'FREE',label:'Plan Free — 10 jours',price:0,statut:'FREE',durationDays:FREE_PLAN_DAYS,target:'Découvrir GLOBAL MARKET gratuitement.',maxUsers:Infinity,maxItems:10,maxCategories:2,maxItemsPerCategory:5,marketplace:false,publicShop:false,limits:['2 catégories maximum','5 produits/services maximum par catégorie','10 produits/services maximum au total'],features:['Accès aux fonctions de gestion hors Marketplace','Essai gratuit pendant 10 jours'],restrictions:['Marketplace non disponible','Produits et services non publiés dans la boutique publique']},
  STANDARD:{code:'STANDARD',label:'Plan Standard — 30 jours',price:STANDARD_PLAN_AMOUNT,statut:'STANDARD',durationDays:STANDARD_PLAN_DAYS,target:'Entreprises souhaitant vendre sur la Marketplace avec un catalogue maîtrisé.',maxUsers:Infinity,maxItems:50,maxCategories:5,maxItemsPerCategory:10,marketplace:true,publicShop:true,limits:['5 catégories maximum','10 produits/services maximum par catégorie','50 produits/services maximum au total'],features:['Accès complet à la Marketplace','Produits et services visibles dans la boutique publique','Fonctions de gestion complètes'],restrictions:[]},
  BUSINESS:{code:'BUSINESS',label:'Plan Business — 1 an',price:BUSINESS_PLAN_AMOUNT,statut:'BUSINESS',durationDays:BUSINESS_PLAN_DAYS,target:'Entreprises souhaitant un accès illimité au programme complet.',maxUsers:Infinity,maxItems:Infinity,maxCategories:Infinity,maxItemsPerCategory:Infinity,marketplace:true,publicShop:true,limits:['Catégories illimitées','Produits et services illimités'],features:['Accès illimité au programme complet','Marketplace et boutique publique','Toutes les fonctions disponibles'],restrictions:[]}
};
function planCode(company){
...
function hasPlanFeature(company,feature=''){
  const p=planDef(company);
  if(feature==='marketplace') return !!p.marketplace;
  if(feature==='public_shop') return !!p.publicShop;
  return true;
}
...
function assertPlanFeature(company,feature='',message=''){
  if(hasPlanFeature(company,feature)) return true;
  const p=planDef(company);
  g3Alert(message||`Cette fonction n’est pas disponible avec ${p.label}. Passez au Plan Standard ou Business pour l’activer.`,'Abonnement requis','warn');
  return false;
}
function subscriptionEndFromToday(days){return new Date(Date.now()+Number(days||0)*86400000).toISOString().slice(0,10)}
function planLimitText(value){return Number.isFinite(value)?String(value):'illimité'}
function companyPlanUsage(d,company){
  const cid=company?.id||'';
  const categories=getCompanyCategoryRecords(d,cid);
  const items=(d.items||[]).filter(i=>i.companyId===cid);
  return {categories,items};
}
function canAddPlanCategory(d,company){
  const p=planDef(company), usage=companyPlanUsage(d,company);
  return !Number.isFinite(p.maxCategories) || usage.categories.length<p.maxCategories;
}
function canAddPlanItem(d,company,cat,currentId=''){
  const p=planDef(company), usage=companyPlanUsage(d,company);
  const otherItems=usage.items.filter(i=>String(i.id||'')!==String(currentId||''));
  const inCategory=otherItems.filter(i=>String(i.cat||'')===String(cat||'')).length;
  if(Number.isFinite(p.maxItems) && otherItems.length>=p.maxItems) return {ok:false,reason:`Votre ${p.label} est limité à ${p.maxItems} produits/services au total.`};
  if(Number.isFinite(p.maxItemsPerCategory) && inCategory>=p.maxItemsPerCategory) return {ok:false,reason:`La catégorie « ${cat} » a atteint la limite de ${p.maxItemsPerCategory} produits/services autorisés avec ${p.label}.`};
  return {ok:true};
}
async function activateCompanyPlan(cid,code){
  ...
  const days=p.durationDays, amount=p.price;
  const confirmed=await g3Confirm(`Activer ${p.label} pour ${c.name} ?\n\nDurée : ${days} jours${amount?`\nMontant : ${money(amount)}`:'\nMontant : Gratuit'}`,'Activation abonnement');
  if(!confirmed) return;
  ...
  d.payments=d.payments||[]; d.payments.push({id:id('pay'),ref:'PAY-'+today().replaceAll('-','')+'-'+randomPart(4),companyId:cid,amount,plan:p.label,status:amount?'Payé':'Gratuit',date:new Date().toISOString(),method:`Activation Super Admin — ${p.label}`});
  save(d); closeSuperModal(); renderSuper(); alert(p.label+' activé avec succès.');
}
function planActivationButtons(cid,currentCode){
  const code=planCode({planCode:currentCode});
  return `<div class="planActivationBox"><h3>Activation abonnement par Super Admin</h3><p>Choisissez la formule à activer. Free : 10 jours. Standard : 30 jours. Business : 1 an (365 jours).</p><div class="planButtons"><button class="${code==='FREE'?'active':''}" onclick="activateCompanyPlan('${cid}','FREE')">FREE — 10 jours</button><button class="${code==='STANDARD'?'active':''}" onclick="activateCompanyPlan('${cid}','STANDARD')">STANDARD — ${money(STANDARD_PLAN_AMOUNT)}/mois</button><button class="${code==='BUSINESS'?'active':''}" onclick="activateCompanyPlan('${cid}','BUSINESS')">BUSINESS — ${money(BUSINESS_PLAN_AMOUNT)}/an</button></div></div>`;
}

let FREE_PLAN_REMINDER_TIMER=null;
let FREE_PLAN_INITIAL_REMINDER=null;
function closeFreePlanUpgradePopup(){document.querySelector('.freePlanUpgradeBackdrop')?.remove()}
function showFreePlanUpgradePopup(){
  ...
  box.innerHTML=`<div class="freePlanUpgradeModal">
    ...
    <span class="freePlanUpgradeBadge">PLAN FREE • 10 JOURS</span>
    <h2 id="freePlanUpgradeTitle">Passez à une formule payante</h2>
    <p class="freePlanUpgradeLead">Votre formule Free est limitée à 2 catégories et 5 produits/services par catégorie, sans accès à la Marketplace. Activez Standard ou Business pour publier votre catalogue dans la boutique publique.</p>
    <div class="freePlanUpgradeStats">
      <div><small>Plan actuel</small><b>Free — 10 jours</b></div>
      <div><small>Durée restante</small><b>${info.left} jour(s)</b></div>
      <div><small>Standard</small><b>${money(STANDARD_PLAN_AMOUNT)} / mois</b></div>
      <div><small>Business</small><b>${money(BUSINESS_PLAN_AMOUNT)} / an</b></div>
    </div>
    <div class="freePlanUpgradeBenefits"><span>✓ Standard : 5 catégories, 10 éléments par catégorie + Marketplace</span><span>✓ Business : programme complet, catalogue illimité + Marketplace</span><span>✓ Vos données existantes sont conservées lors du changement de formule</span></div>
    <p class="freePlanUpgradeNote">Ce rappel est affiché automatiquement toutes les 15 minutes pendant l’utilisation du Plan Free.</p>
    <div class="freePlanUpgradeActions freePlanUpgradeActionsThree"><button type="button" class="freePlanUnderstood" onclick="closeFreePlanUpgradePopup()">Plus tard</button><button type="button" class="freePlanBuy" onclick="closeFreePlanUpgradePopup();openSubscriptionPayment('STANDARD')">Choisir Standard</button><button type="button" class="freePlanBuy" onclick="closeFreePlanUpgradePopup();openSubscriptionPayment('BUSINESS')">Choisir Business</button></div>
  </div>`;
  document.body.appendChild(box);
}
function manageFreePlanReminder(){
  const {user,company}=current();
  const free=Boolean(user&&user.role!=='superadmin'&&company&&planCode(company)==='FREE');
  if(!free){
    if(FREE_PLAN_REMINDER_TIMER){clearInterval(FREE_PLAN_REMINDER_TIMER);FREE_PLAN_REMINDER_TIMER=null;}
    if(FREE_PLAN_INITIAL_REMINDER){clearTimeout(FREE_PLAN_INITIAL_REMINDER);FREE_PLAN_INITIAL_REMINDER=null;}
    closeFreePlanUpgradePopup(); return;
  }
  if(!FREE_PLAN_REMINDER_TIMER){
    FREE_PLAN_INITIAL_REMINDER=setTimeout(()=>{FREE_PLAN_INITIAL_REMINDER=null;showFreePlanUpgradePopup();},1000);
    FREE_PLAN_REMINDER_TIMER=setInterval(()=>showFreePlanUpgradePopup(),15*60*1000);
  }
}
...
/* === Notifications temps réel légères : commandes et messages === */
let GM_NOTIFY_IDENTITY='';
let GM_NOTIFY_ORDER_SIGNATURES=new Map();
let GM_NOTIFY_MESSAGE_IDS=new Set();
let GM_NOTIFY_QUEUE=[];
let GM_NOTIFY_VISIBLE=false;
let GM_NOTIFY_POLL_BUSY=false;

function gmNotificationOrderSignature(order){
  return [
    String(order?.validationStatus||''),
    String(order?.paymentStatus||''),
    String(order?.deliveryStatus||''),
    String(order?.afterSaleStatus||''),
    order?.clientCancelled?'1':'0'
  ].join('|');
}
function gmNotificationOrderClientText(order){
  ...
  const after=String(order?.afterSaleStatus||'').toLowerCase();
  if(validation.includes('annul')||delivery.includes('annul')||after.includes('annul')||order?.clientCancelled) return 'Votre commande est annulée.';
  if(delivery.includes('livr')&&!delivery.includes('cours')) return 'Votre commande est livrée.';
  if(payment.includes('confirm')||delivery.includes('cours')) return 'Paiement confirmé : votre commande est en cours de livraison.';
  if(validation.includes('valid')) return 'Votre commande est confirmée.';
  return '';
}
function gmResetNotificationBaseline(payload){
  GM_NOTIFY_ORDER_SIGNATURES=new Map((payload?.orders||[]).map(order=>[String(order.id||''),gmNotificationOrderSignature(order)]));
  GM_NOTIFY_MESSAGE_IDS=new Set((payload?.messages||[]).map(message=>String(message.id||'')));
}
function gmQueueRealtimeNotice(notice){
  if(!notice)return;
  GM_NOTIFY_QUEUE.push(notice);
  gmShowNextRealtimeNotice();
}
function gmCloseRealtimeNotice(){
  document.getElementById('gmRealtimeNotice')?.remove();
  GM_NOTIFY_VISIBLE=false;
  setTimeout(gmShowNextRealtimeNotice,80);
}
function gmShowNextRealtimeNotice(){
  if(GM_NOTIFY_VISIBLE||!GM_NOTIFY_QUEUE.length)return;
  const notice=GM_NOTIFY_QUEUE.shift();
  GM_NOTIFY_VISIBLE=true;
  const icon=notice.kind==='message'?'💬':'🛒';
  document.body.insertAdjacentHTML('beforeend',`<div class="gmRealtimeNoticeBackdrop" id="gmRealtimeNotice"><section class="gmRealtimeNoticeCard"><button type="button" class="gmRealtimeNoticeClose" onclick="gmCloseRealtimeNotice()" aria-label="Fermer">×</button><div class="gmRealtimeNoticeIcon">${icon}</div><span class="gmRealtimeNoticeKicker">GLOBAL MARKET</span><h2>${esc(notice.title||'Nouvelle notification')}</h2><p>${esc(notice.text||'')}</p><div class="gmRealtimeNoticeActions"><button type="button" class="gmRealtimeNoticeLater" onclick="gmCloseRealtimeNotice()">Fermer</button><button type="button" class="gmRealtimeNoticeOpen" onclick="gmOpenRealtimeNotice()">${esc(notice.button||'Voir')}</button></div></section></div>`);
  window.GM_ACTIVE_REALTIME_NOTICE=notice;
}
async function gmOpenRealtimeNotice(){
  const notice=window.GM_ACTIVE_REALTIME_NOTICE;
  gmCloseRealtimeNotice();
  if(!notice)return;
  try{
    if(notice.mode==='admin'){
      if(CLOUD_SAVE_PENDING&&CLOUD_SESSION) await cloudSaveNow(CLOUD_DATA).catch(()=>{});
      await cloudLoadData();
      if(notice.kind==='order'){
        showMarketplacePage();
        setTimeout(()=>openMarketplaceOrderPopup(notice.orderId,true),60);
      }else{
        showMarketplaceAdminPage('messages');
        setTimeout(()=>openAdminMessageThread(notice.threadId),80);
      }
    }else{
      await cloudLoadPublicData();
      if(notice.kind==='order'){
        openClientSpace();
        setTimeout(()=>openMarketplaceOrderPopup(notice.orderId,false),60);
      }else{
        openClientMessagesPopup();
      }
    }
  }catch(error){
    console.warn('Ouverture notification impossible',error);
  }
}
async function gmPollRealtimeNotifications(){
  if(GM_NOTIFY_POLL_BUSY||document.visibilityState==='hidden')return;
  const adminMode=Boolean(CLOUD_SESSION&&CLOUD_SESSION.role==='admin'&&CLOUD_SESSION.companyId);
  const clientMode=Boolean(!adminMode&&PUBLIC_CLIENT_SESSION?.clientId);
  if(!adminMode&&!clientMode){GM_NOTIFY_IDENTITY='';GM_NOTIFY_ORDER_SIGNATURES.clear();GM_NOTIFY_MESSAGE_IDS.clear();return;}
  GM_NOTIFY_POLL_BUSY=true;
  try{
    const url=adminMode?'/api/notifications':'/api/public/notifications';
    const response=await fetchWithTimeout(url,{cache:'no-store'},8000);
    const payload=await readApiPayload(response);
    const identity=String(payload.identity||'');
    if(!identity)return;
    if(GM_NOTIFY_IDENTITY!==identity){
      GM_NOTIFY_IDENTITY=identity;
      gmResetNotificationBaseline(payload);
      return;
    }
    const currentOrders=new Map();
    for(const order of payload.orders||[]){
      const orderId=String(order.id||'');
      const signature=gmNotificationOrderSignature(order);
      currentOrders.set(orderId,signature);
      const oldSignature=GM_NOTIFY_ORDER_SIGNATURES.get(orderId);
      if(adminMode){
        if(oldSignature===undefined){
          gmQueueRealtimeNotice({mode:'admin',kind:'order',orderId,title:'Vous avez une nouvelle commande',text:`Nouvelle commande${order.client?' de '+order.client:''}.`,button:'Voir la commande'});
        }
      }else if(oldSignature!==undefined&&oldSignature!==signature){
        const text=gmNotificationOrderClientText(order);
        if(text)gmQueueRealtimeNotice({mode:'client',kind:'order',orderId,title:'Statut de commande mis à jour',text,button:'Voir la commande'});
      }
    }
    GM_NOTIFY_ORDER_SIGNATURES=currentOrders;
    const currentMessages=new Set();
    for(const message of payload.messages||[]){
      const messageId=String(message.id||'');currentMessages.add(messageId);
      if(!GM_NOTIFY_MESSAGE_IDS.has(messageId)){
        gmQueueRealtimeNotice({mode:adminMode?'admin':'client',kind:'message',threadId:String(message.threadId||message.id||''),title:'Vous avez un nouveau message',text:message.subject?`Objet : ${message.subject}`:'Un nouveau message vient d’arriver.',button:'Voir le message'});
      }
    }
    GM_NOTIFY_MESSAGE_IDS=currentMessages;
  }catch(error){
    if(![401,403].includes(Number(error?.status||0)))console.warn('Vérification notifications indisponible',error?.message||error);
  }finally{GM_NOTIFY_POLL_BUSY=false;}
}
if(typeof window!=='undefined'){
  window.setInterval(()=>gmPollRealtimeNotifications(),12000);
  window.setTimeout(()=>gmPollRealtimeNotifications(),2500);
  document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible')gmPollRealtimeNotifications();});
}
...
async function gmRefreshPublicDataAfterAction(){try{return await cloudLoadPublicData()}catch(error){console.warn('Actualisation publique différée après action confirmée',error?.message||error);return CLOUD_DATA}}
...
function logout(){if(FREE_PLAN_REMINDER_TIMER){clearInterval(FREE_PLAN_REMINDER_TIMER);FREE_PLAN_REMINDER_TIMER=null;} if(FREE_PLAN_INITIAL_REMINDER){clearTimeout(FREE_PLAN_INITIAL_REMINDER);FREE_PLAN_INITIAL_REMINDER=null;} closeFreePlanUpgradePopup(); cloudClearSession().finally(()=>{location.hash='#home';cloudLoadPublicData().catch(()=>{}).finally(()=>renderGlobalShop())})}
...
________________________________________
3. ABONNEMENTS
GLOBAL MARKET propose trois formules : Free, Standard et Business.
PLAN FREE
Le Plan Free est gratuit pendant dix (10) jours. Il autorise au maximum deux (2) catégories et cinq (5) produits ou services par catégorie, soit dix (10) éléments au total. Il ne donne pas accès à la Marketplace et les produits/services de l’entreprise ne sont pas publiés dans la boutique publique.
PLAN STANDARD
Le Plan Standard est facturé 4 800 FCFA par mois pour une période d’activation de trente (30) jours. Il autorise au maximum cinq (5) catégories et dix (10) produits ou services par catégorie, soit cinquante (50) éléments au total. Il donne accès à la Marketplace et à la publication dans la boutique publique.
PLAN BUSINESS
Le Plan Business est facturé 46 100 FCFA par an pour une période d’activation de trois cent soixante-cinq (365) jours. Il donne un accès illimité au programme complet, y compris la Marketplace et la boutique publique.
Chaque abonnement possède :
- une durée fixe ;
- un statut d’activation ;
- une date de début ;
- une date d’expiration.
MEGA SERVICES SARL U peut améliorer les fonctionnalités de la plateforme tout en appliquant les limites et droits correspondant à chaque formule active.
________________________________________
...
function legacyDup5_requestPasswordReset(){
  const d=seed();
...
async function legacyDup13_resetPasswordRequestByAdmin(rid){
...
async function legacyDup6_enforcePasswordChange(u){
...
async function legacyDup3_login(){
...
async function legacyDup4_registerCompany(){
  ...
  d.companies.push({id:cid,name,legalForm,rccm,taxAccount,activity,owner,address,phone,email,businessType:type,status:'FREE',planCode:'FREE',plan:'Plan Free — 10 jours',subscriptionStart:today(),subscriptionEnd:new Date(Date.now()+FREE_PLAN_DAYS*86400000).toISOString().slice(0,10),createdAt:new Date().toISOString(),notes:'',shopSlug:slugify(name),shopBanner:'Boutique officielle',shopColor:'#024644'});
...
function legacyDup19_currentGlobalClient(){
...
function legacyDup21_openCurrentGlobalClientSpace(){
  const companyId=PUBLIC_CLIENT_SESSION?.companyId||'';
  if(!companyId) return openGlobalClientAuth('login');
  openClientSpace(companyId);
}
function legacyDup22_logoutCurrentGlobalClient(){
  const companyId=PUBLIC_CLIENT_SESSION?.companyId||'';
  logoutPublicClient(companyId);
}
function legacyDup23_globalUniversalHeader(active='public',options={}){
  const {user}=current();
  ...
  if(user){
    if(!restricted){
      if(user.role==='superadmin'){
        nav+=`<button type="button" class="${active==='super'?'active':''}" onclick="openSuperAdministration()">▦ Administration</button><button type="button" class="${active==='super-orders'?'active':''}" onclick="openSuperOrderProcessing()">📦 Traitement des commandes</button><button type="button" onclick="exportData()">📤 Exporter données</button>`;
      }else{
    ...
    actions=`<div class="gmUniversalUser"><span class="gmUniversalIdentity">👤 <b>Compte client</b><small>Session sécurisée</small></span><button type="button" class="gmUniversalLogout" onclick="logoutCurrentGlobalClient()">Déconnexion</button></div>`;
  }else{
...
async function legacyDup9_saveAccountUser(uid){
...
function gmDashPeriodValue(){
  const allowed=['today','yesterday','7d','30d','month','year'];
  const value=String(window.GM_DASHBOARD_PERIOD||'today');
  return allowed.includes(value)?value:'today';
}
function gmDashDateFromKey(key){return new Date(String(key)+'T12:00:00')}
function gmDashKeyFromDate(date){return new Date(date).toISOString().slice(0,10)}
function gmDashRangeRows(rows,start,end){
  const a=String(start||''),b=String(end||'');
  return (rows||[]).filter(row=>{const key=gmDashDateKey(row.date||row.createdAt);return key&&key>=a&&key<=b});
}
function gmDashPeriodMeta(rows=[],manageYear=getManageYear()){
  const value=gmDashPeriodValue(),now=gmDashDateFromKey(gmDashDayKey(0));
  let start=new Date(now),end=new Date(now),prevStart=new Date(now),prevEnd=new Date(now),label='Aujourd’hui',compare='vs hier';
  if(value==='yesterday'){
    start.setDate(start.getDate()-1);end=new Date(start);prevStart=new Date(start);prevStart.setDate(prevStart.getDate()-1);prevEnd=new Date(prevStart);label='Hier';compare='vs veille';
  }else if(value==='7d'||value==='30d'){
    const days=value==='7d'?7:30;start.setDate(start.getDate()-(days-1));prevEnd=new Date(start);prevEnd.setDate(prevEnd.getDate()-1);prevStart=new Date(prevEnd);prevStart.setDate(prevStart.getDate()-(days-1));label=days===7?'7 derniers jours':'30 derniers jours';compare=`vs ${days} jours précédents`;
  }else if(value==='month'){
    start=new Date(now.getFullYear(),now.getMonth(),1,12);end=new Date(now);prevStart=new Date(now.getFullYear(),now.getMonth()-1,1,12);prevEnd=new Date(now.getFullYear(),now.getMonth(),0,12);label='Ce mois';compare='vs mois précédent';
  }else if(value==='year'){
    const y=Number(manageYear||now.getFullYear());start=new Date(y,0,1,12);end=new Date(y,11,31,12);prevStart=new Date(y-1,0,1,12);prevEnd=new Date(y-1,11,31,12);label=`Année ${y}`;compare=`vs ${y-1}`;
  }else{
    prevStart=new Date(now);prevStart.setDate(prevStart.getDate()-1);prevEnd=new Date(prevStart);
  }
  const startKey=gmDashKeyFromDate(start),endKey=gmDashKeyFromDate(end),prevStartKey=gmDashKeyFromDate(prevStart),prevEndKey=gmDashKeyFromDate(prevEnd);
  return {value,label,compare,startKey,endKey,prevStartKey,prevEndKey,rows:gmDashRangeRows(rows,startKey,endKey),prevRows:gmDashRangeRows(rows,prevStartKey,prevEndKey)};
}
function gmDashboardSetPeriod(value){
  window.GM_DASHBOARD_PERIOD=String(value||'today');
  renderDash('home');
}
function gmDashboardFilter(value){
  const q=String(value||'').trim().toLowerCase();
  document.querySelectorAll('#home [data-dashboard-search]').forEach(el=>{
    const hay=String(el.getAttribute('data-dashboard-search')||'').toLowerCase();
...
function gmDashboardHomeHTML(ctx={}){
  ...
  const period=gmDashPeriodMeta(sales,manageYear);
  ...
  const periodCA=gmDashSum(period.rows,'total'),prevPeriodCA=gmDashSum(period.prevRows,'total');
  const periodProfit=gmDashSum(period.rows,'profit'),prevPeriodProfit=gmDashSum(period.prevRows,'profit');
  const periodOrders=period.rows.length,prevPeriodOrders=period.prevRows.length,periodClients=gmDashActiveClients(period.rows),prevPeriodClients=gmDashActiveClients(period.prevRows);
  const stockQty=gmDashStockQty(items),yesterdayStock=stockSeries.length>1?stockSeries[stockSeries.length-2]:stockQty;
  const caChange=gmDashPct(periodCA,prevPeriodCA),orderChange=gmDashPct(periodOrders,prevPeriodOrders),clientChange=gmDashPct(periodClients,prevPeriodClients),stockChange=gmDashPct(stockQty,yesterdayStock),profitChange=gmDashPct(periodProfit,prevPeriodProfit);
  const kpis=[
    {tone:'green',icon:'↗',label:'Chiffre d\'affaires',value:money(periodCA),change:caChange,compare:period.compare,color:'#16A05D',series:caSeries},
    {tone:'blue',icon:'🛒',label:'Commandes',value:periodOrders.toLocaleString('fr-FR'),change:orderChange,compare:period.compare,color:'#2684E8',series:ordersSeries},
    {tone:'violet',icon:'👥',label:'Clients actifs',value:periodClients.toLocaleString('fr-FR'),change:clientChange,compare:period.compare,color:'#8A4DE8',series:clientSeries},
    ...
    {tone:'gold',icon:'₣',label:'Bénéfice',value:money(periodProfit),change:profitChange,compare:period.compare,color:'#F2B632',series:profitSeries},
    {tone:'turquoise',icon:'➚',label:'Croissance',value:gmDashPctText(caChange),change:null,compare:period.compare,color:'#159C9C',series:growthSeries}
  ];
  ...
  const stockMoves=gmDashStockMovementRows(d,cid);
...
async function legacyDup2_deleteStockCategoryFromPopup(cat){
...
function recalculateCompanySalesFromCurrentChargeRates(d,companyId){
  const items=(d.items||[]).filter(i=>i.companyId===companyId);
  const byId=new Map(items.map(i=>[String(i.id||''),i]));
  let updated=0;
  (d.sales||[]).forEach(s=>{
    if(s.companyId!==companyId) return;
    const item=byId.get(String(s.itemId||'')) || items.find(i=>String(i.name||'')===String(s.name||''));
    if(!item || isBoutiqueItem(item)) return;
    const rate=Math.max(0,Math.min(100,Number(item.charge||0)));
    const total=Number(s.total||0);
    const fee=Number(s.serviceFee||0);
    let base=Number(s.serviceBasePrice);
    if(!Number.isFinite(base) || base<0) base=Math.max(0,total-fee);
    if(base===0 && total>0) base=Math.max(0,total-fee);
    const charges=base*(rate/100);
    s.serviceBasePrice=base;
    s.charges=charges;
    s.profit=total-charges;
    s.chargeRate=rate;
    updated++;
  });
  return updated;
...
function saveChargePercentages(){
  if(!requireAdmin()) return;
  const {d,company}=current();
  document.querySelectorAll('.chargeInput[data-id]').forEach(inp=>{
    const it=d.items.find(i=>i.id===inp.dataset.id&&i.companyId===company.id);
    if(it&&!isBoutiqueItem(it)) it.charge=Math.max(0,Math.min(100,+inp.value||0));
  });
  const updatedSales=recalculateCompanySalesFromCurrentChargeRates(d,company.id);
  save(d);
  alert('Pourcentages enregistrés. '+updatedSales+' vente(s) de service mise(s) à niveau automatiquement dans Rapports et dans le Rapport bilan détaillé. Les produits BOUTIQUE restent calculés automatiquement avec le prix d’achat.');
  renderDash('param');
}
...
async function legacyDeleteCategory(cat){
  if(!requireAdmin()) return;
  const {d,company}=current();
...
async function legacyDup1_addItem(){
  if(!requireAdmin('La caisse ne peut pas gérer les stocks.')) return;
  if(!ensureActiveExerciseEditable()) return;
...
async function legacyDup12_resetUserPasswordDirect(uid){
  if(!requireAdmin('Réservé à l’administrateur.')) return;
  const {d,company}=current(); const u=(d.users||[]).find(x=>x.id===uid&&x.companyId===company.id);
...
async function legacyDup8_addUser(){
...
function legacyDup11_blockUser(uid){const d=seed(), u=d.users.find(x=>x.id===uid); if(u)u.status='blocked'; save(d); renderDash('param')}


function gmWhatsappDispatchLabel(order){
  const raw=String(order?.whatsappDispatchStatus||'').trim();
  if(raw)return raw;
  return Array.isArray(order?.whatsappRecipients)&&order.whatsappRecipients.length?'À envoyer':'Numéro non configuré';
}
function gmWhatsappDispatchClass(order){
  const value=gmWhatsappDispatchLabel(order).toLowerCase();
  if(value.includes('envoy'))return 'sent';
  if(value.includes('partiel'))return 'partial';
  if(value.includes('échec')||value.includes('echec'))return 'failed';
  if(value.includes('config')||value.includes('numéro')||value.includes('numero'))return 'config';
  return 'pending';
}
function openSuperAdministration(){location.hash='#super-admin';renderSuper();}
function openSuperOrderProcessing(){location.hash='#super-commandes';renderSuperOrderProcessing();}
function superOrderProcessingValue(order){return String(order?.globalProcessingStatus||'Nouvelle').trim()||'Nouvelle';}
function superOrderProcessingOptions(order){
  const currentStatus=superOrderProcessingValue(order);
  return ['Nouvelle','En traitement','Transmise à la boutique','Incident','Clôturée'].map(v=>`<option value="${esc(v)}" ${currentStatus===v?'selected':''}>${esc(v)}</option>`).join('');
}
function renderSuperOrderProcessing(){
  const {d,user}=current();
  if(!user||user.role!=='superadmin')return render();
  const companyMap=new Map((d.companies||[]).map(c=>[String(c.id),c]));
  const orders=(d.orders||[]).filter(o=>!o.adminDeleted).slice().sort((a,b)=>new Date(b.date||0)-new Date(a.date||0));
  const newCount=orders.filter(o=>superOrderProcessingValue(o)==='Nouvelle').length;
  const pendingWhatsapp=orders.filter(o=>!gmWhatsappDispatchLabel(o).toLowerCase().includes('envoy')).length;
  const total=orders.reduce((sum,o)=>sum+Number(orderTotal(o)||0),0);
  const rows=orders.map(o=>{
    const company=companyMap.get(String(o.companyId))||{};
    const numbers=marketplaceWhatsappNumbers(company);
    const wa=gmWhatsappDispatchLabel(o);
    return `<tr class="superOrderRow" data-search="${esc(((o.id||'')+' '+(o.checkoutId||'')+' '+(o.client||'')+' '+(o.clientPhone||'')+' '+(o.shopName||company.name||'')+' '+numbers.join(' ')+' '+wa+' '+superOrderProcessingValue(o)).toLowerCase())}" data-processing="${esc(superOrderProcessingValue(o).toLowerCase())}" data-whatsapp="${esc(gmWhatsappDispatchClass(o))}">
      <td><button class="orderLinkBtn" onclick="openSuperOrderDetails('${esc(o.id)}')">#${esc(o.id||'CMD')}</button><small>${esc(o.checkoutId||'')}</small></td>
      <td>${o.date?new Date(o.date).toLocaleString('fr-FR'):'-'}</td>
      <td><b>${esc(o.client||'Client')}</b><small>${esc(o.clientPhone||'')}</small></td>
      <td><b>${esc(o.shopName||company.name||'Boutique')}</b><small>${esc(numbers.join(' · ')||'Aucun WhatsApp')}</small></td>
      <td>${orderItemsCount(o)} article(s)<small>${money(orderTotal(o))}</small></td>
      <td><span class="mkStatus">${esc(orderMainStatus(o))}</span></td>
      <td><span class="waDispatchBadge ${gmWhatsappDispatchClass(o)}">${esc(wa)}</span>${o.whatsappLastAttemptAt?`<small>${new Date(o.whatsappLastAttemptAt).toLocaleString('fr-FR')}</small>`:''}</td>
      <td><select class="superOrderStatusSelect" onchange="superSetOrderProcessingStatus('${esc(o.id)}',this.value)">${superOrderProcessingOptions(o)}</select></td>
      <td><div class="superOrderActions"><button onclick="openSuperOrderDetails('${esc(o.id)}')">Détails</button><button class="waOrderBtn" onclick="superDispatchOrderWhatsapp('${esc(o.id)}')">${wa.toLowerCase().includes('envoy')?'Renvoyer WhatsApp':'Envoyer WhatsApp'}</button></div></td>
    </tr>`;
  }).join('')||'<tr><td colspan="9">Aucune commande disponible.</td></tr>';
  app.innerHTML=globalUniversalHeader('super-orders')+`<div class="superShell superUnifiedShell"><aside class="superSide"><div class="superBrand"><div class="superLogo">MS</div><div><h2>MEGA SERVICES</h2><p>Super Admin GLOBAL MARKET</p></div></div><div class="superMenu"><button onclick="openSuperAdministration()">📊 Vue générale</button><button class="active" onclick="openSuperOrderProcessing()">📦 Traitement des commandes</button><button onclick="exportData()">📤 Exporter données</button><button class="danger" onclick="logout()">🚪 Déconnexion</button></div><div class="superNote">Centre de supervision des commandes Marketplace et de leur transmission WhatsApp aux boutiques.</div></aside><main class="superMain"><div class="superHero"><div><span class="superKicker">COMMANDES GLOBAL MARKET CI</span><h1>Traitement des commandes</h1><p>Suivez chaque lot boutique, contrôlez sa transmission WhatsApp et intervenez en cas d’échec.</p></div><button class="superExport" onclick="openSuperOrderProcessing()">↻ Actualiser</button></div><div class="superStats superOrderStats"><div class="superStat"><span>🛒</span><small>Commandes</small><b>${orders.length}</b></div><div class="superStat"><span>🆕</span><small>Nouvelles</small><b>${newCount}</b></div><div class="superStat"><span>💬</span><small>WhatsApp à vérifier</small><b>${pendingWhatsapp}</b></div><div class="superStat"><span>💰</span><small>Valeur commandes</small><b>${money(total)}</b></div></div><section class="superPanel superOrdersPanel"><div class="superPanelHead"><div><h2>Centre de traitement</h2><p>Une commande multi-boutiques apparaît ici sous forme de lots séparés, un lot par boutique concernée.</p></div><span>${orders.length} lot(s)</span></div><div class="superOrderFilters"><input id="superOrderSearch" placeholder="N° commande, client, téléphone, boutique, WhatsApp..." oninput="filterSuperOrderProcessing()"><select id="superOrderProcessingFilter" onchange="filterSuperOrderProcessing()"><option value="">Tous les traitements</option><option value="nouvelle">Nouvelles</option><option value="en traitement">En traitement</option><option value="transmise à la boutique">Transmises</option><option value="incident">Incidents</option><option value="clôturée">Clôturées</option></select><select id="superOrderWhatsappFilter" onchange="filterSuperOrderProcessing()"><option value="">Tous WhatsApp</option><option value="sent">Envoyées</option><option value="pending">À envoyer</option><option value="partial">Partielles</option><option value="failed">Échecs</option><option value="config">Configuration requise</option></select></div><div class="superTableWrap"><table class="superTable superOrdersTable"><thead><tr><th>Commande</th><th>Date</th><th>Client</th><th>Boutique / WhatsApp</th><th>Articles / Montant</th><th>Commande</th><th>Transmission WhatsApp</th><th>Traitement GLOBAL MARKET</th><th>Actions</th></tr></thead><tbody>${rows}</tbody></table></div></section></main></div>`;
  setTimeout(initFlexibleHorizontalMenu,0);
}
function filterSuperOrderProcessing(){
  const q=String(document.getElementById('superOrderSearch')?.value||'').trim().toLowerCase();
  const processing=String(document.getElementById('superOrderProcessingFilter')?.value||'').toLowerCase();
  const whatsapp=String(document.getElementById('superOrderWhatsappFilter')?.value||'').toLowerCase();
  document.querySelectorAll('.superOrderRow').forEach(row=>{
    const okQ=!q||String(row.dataset.search||'').includes(q);
    const okP=!processing||String(row.dataset.processing||'')===processing;
    const okW=!whatsapp||String(row.dataset.whatsapp||'')===whatsapp;
    row.style.display=okQ&&okP&&okW?'':'none';
  });
}
async function superSetOrderProcessingStatus(orderId,status){
  try{
    await secureEmployeePost('/api/super/orders/action',{orderId,action:'set_processing_status',status});
    const d=seed(),o=(d.orders||[]).find(x=>String(x.id)===String(orderId));if(o)o.globalProcessingStatus=status;
  }catch(error){alert(secureErrorMessage(error,'Mise à jour du traitement impossible.'));await cloudLoadData();renderSuperOrderProcessing();}
}
function marketplaceOrderWhatsappManualMessage(order,company){
  const lines=normalizeOrderItems(order).slice(0,20).map((line,index)=>`${index+1}. ${String(line.item||'Article')} x${Number(line.qty||1)} — ${money(Number(line.total||0))}`);
  const delivery=[String(order?.deliveryCity||'').trim(),String(order?.deliveryNeighborhood||'').trim(),String(order?.deliveryAddressDetail||'').trim()].filter(Boolean).join(' — ')||'-';
  return [
    ...
    `Livraison : ${delivery}`,
    `Mode de livraison : ${String(order?.shippingMethod||'-')}`,
    `Paiement : ${String(order?.paymentMethod||'Non choisi')}`,
    '',
    'Connectez-vous à GLOBAL MARKET CI pour traiter la commande.'
  ].join('\n');
}
function openManualWhatsappUrl(number,message){
  const clean=normalizeMarketplaceWhatsappNumber(number);
  if(!clean)return alert('Numéro WhatsApp invalide.');
  const url=`https://wa.me/${encodeURIComponent(clean)}?text=${encodeURIComponent(message||'')}`;
  const win=window.open(url,'_blank','noopener,noreferrer');
  if(!win)location.href=url;
}
async function markSuperWhatsappManualSent(orderId,number){
  const btn=typeof event!=='undefined'?event.currentTarget:null;
  if(btn){btn.disabled=true;btn.textContent='Enregistrement…';}
  try{
    await secureEmployeePost('/api/super/orders/action',{orderId,action:'mark_whatsapp_manual_sent',recipient:number});
    await cloudLoadData();
    document.getElementById('manualWhatsappOrderModal')?.remove();
    renderSuperOrderProcessing();
    alert('Envoi WhatsApp marqué comme effectué.');
  }catch(error){if(btn?.isConnected){btn.disabled=false;btn.textContent='Marquer comme envoyé';}alert(secureErrorMessage(error,'Impossible de confirmer l’envoi WhatsApp.'));}
}
function superDispatchOrderWhatsapp(orderId){
  document.getElementById('manualWhatsappOrderModal')?.remove();
  const d=seed(),order=(d.orders||[]).find(x=>String(x.id)===String(orderId));
  if(!order)return alert('Commande introuvable.');
  const company=(d.companies||[]).find(c=>String(c.id)===String(order.companyId))||{};
  const numbers=marketplaceWhatsappNumbers(company);
  if(!numbers.length)return alert('Aucun numéro WhatsApp n’est enregistré pour cette boutique. Demandez à son administrateur de renseigner Marketplace > WhatsApp commandes.');
  const message=marketplaceOrderWhatsappManualMessage(order,company);
  const sent=new Set((Array.isArray(order.whatsappSentRecipients)?order.whatsappSentRecipients:[]).map(normalizeMarketplaceWhatsappNumber));
  const recipientRows=numbers.map((number,index)=>`<div class="manualWhatsappRecipient ${sent.has(number)?'done':''}"><div><small>Destinataire ${index+1}</small><b>+${esc(number)}</b>${sent.has(number)?'<span class="manualWaDone">✓ Envoi confirmé</span>':''}</div><div class="manualWhatsappRecipientActions"><button class="waOrderBtn" onclick="openManualWhatsappUrl('${esc(number)}',document.getElementById('manualWhatsappMessage')?.value||'')">Ouvrir WhatsApp</button><button class="btn2" onclick="markSuperWhatsappManualSent('${esc(order.id)}','${esc(number)}')" ${sent.has(number)?'disabled':''}>${sent.has(number)?'Déjà confirmé':'Marquer comme envoyé'}</button></div></div>`).join('');
  document.body.insertAdjacentHTML('beforeend',`<div class="marketPayModalBackdrop" id="manualWhatsappOrderModal"><div class="marketPayModal manualWhatsappOrderBox"><button class="marketPayClose" onclick="document.getElementById('manualWhatsappOrderModal')?.remove()">×</button><h2>Envoi manuel WhatsApp</h2><p><b>Commande #${esc(order.id)}</b> — ${esc(order.shopName||company.name||'Boutique')}</p><div class="manualWhatsappHelp">1. Cliquez sur <b>Ouvrir WhatsApp</b>. 2. Vérifiez puis envoyez le message dans WhatsApp. 3. Revenez ici et cliquez sur <b>Marquer comme envoyé</b>.</div><label>Message préparé<textarea id="manualWhatsappMessage" rows="12">${esc(message)}</textarea></label><div class="manualWhatsappRecipients">${recipientRows}</div><div class="marketPayActions"><button class="btn2" onclick="document.getElementById('manualWhatsappOrderModal')?.remove()">Fermer</button></div></div></div>`);
}
function openSuperOrderDetails(orderId){
  document.getElementById('superOrderDetailsModal')?.remove();
  ...
  const numbers=marketplaceWhatsappNumbers(company);
  const rows=normalizeOrderItems(o).map((line,i)=>`<tr><td>${i+1}</td><td>${esc(line.item||'Article')}</td><td>${Number(line.qty||1)}</td><td>${money(line.unit||0)}</td><td>${money(line.total||0)}</td></tr>`).join('');
  document.body.insertAdjacentHTML('beforeend',`<div class="marketPayModalBackdrop" id="superOrderDetailsModal"><div class="marketPayModal orderDetailsBox superOrderDetailsBox"><button class="marketPayClose" onclick="document.getElementById('superOrderDetailsModal')?.remove()">×</button><h2>Traitement commande #${esc(o.id)}</h2><div class="superOrderDetailGrid"><div><small>Boutique</small><b>${esc(o.shopName||company.name||'Boutique')}</b></div><div><small>WhatsApp boutique</small><b>${esc(numbers.join(' · ')||'Non configuré')}</b></div><div><small>Client</small><b>${esc(o.client||'Client')} · ${esc(o.clientPhone||'-')}</b></div><div><small>Transmission</small><b>${esc(gmWhatsappDispatchLabel(o))}</b></div><div><small>Traitement GLOBAL MARKET</small><b>${esc(superOrderProcessingValue(o))}</b></div><div><small>Total</small><b>${money(orderTotal(o))}</b></div></div><table class="mkOrdersTable"><tr><th>N°</th><th>Produit / service</th><th>Qté</th><th>PU</th><th>Total</th></tr>${rows}</table>${o.whatsappLastError?`<p class="notice waErrorNotice"><b>Dernier retour WhatsApp :</b> ${esc(o.whatsappLastError)}</p>`:''}<div class="marketPayActions"><button class="waOrderBtn" onclick="document.getElementById('superOrderDetailsModal')?.remove();superDispatchOrderWhatsapp('${esc(o.id)}')">Envoyer / Renvoyer WhatsApp</button><button class="btn2" onclick="document.getElementById('superOrderDetailsModal')?.remove()">Fermer</button></div></div></div>`);
}
...
async function legacyDup14_resetPasswordRequestBySuper(rid){
  const {d,user}=current();
...
async function legacyDup15_superResetAdminPassword(uid){
  const {d,user}=current();
  if(user?.role!=='superadmin') return alert('Réservé au Super Admin GLOBAL MARKET.');
...
async function renewCompany(cid){const d=seed(), c=d.companies.find(x=>x.id===cid); if(!c)return; const p=planDef(c), days=p.durationDays, amount=p.price; const ok=await g3Confirm(`Renouveler ${p.label} pour ${c.name} ?\n\nDurée : ${days} jours\nMontant : ${money(amount)}`,'Renouvellement abonnement'); if(!ok)return; c.status=p.code; c.planCode=p.code; c.plan=p.label; c.subscriptionStart=today(); c.subscriptionEnd=subscriptionEndFromToday(days); d.payments=d.payments||[]; d.payments.push({id:id('pay'),ref:'PAY-'+today().replaceAll('-','')+'-'+randomPart(4),companyId:cid,amount,plan:p.label,status:amount?'Payé':'Gratuit',date:new Date().toISOString(),method:'Renouvellement Super Admin'}); save(d); renderSuper()}
...
function availableSubscriptionPlansHTML(){
  const plans=[
    {code:'FREE',name:'PLAN FREE',price:0,duration:'10 jours',status:'FREE',tag:'Découverte',features:['Gratuit pendant 10 jours','2 catégories maximum','5 produits/services par catégorie','10 produits/services maximum','Sans accès Marketplace ni boutique publique']},
    {code:'STANDARD',name:'PLAN STANDARD',price:STANDARD_PLAN_AMOUNT,duration:'mois',status:'STANDARD',tag:'Marketplace incluse',features:['5 catégories maximum','10 produits/services par catégorie','50 produits/services maximum','Accès complet à la Marketplace','Catalogue visible dans la boutique publique']},
    {code:'BUSINESS',name:'PLAN BUSINESS',price:BUSINESS_PLAN_AMOUNT,duration:'an',status:'BUSINESS',tag:'Accès illimité',features:['Programme complet sans limite de catalogue','Catégories illimitées','Produits et services illimités','Marketplace et boutique publique','Accès à toutes les fonctionnalités']}
  ];
  return `<div class="g2panel subscriptionPlansBox"><div class="subscriptionPlansHead"><div><h3>Choisissez votre formule GLOBAL MARKET</h3><p>Free pour découvrir, Standard pour vendre sur la Marketplace, Business pour un accès illimité.</p></div><span>GLOBAL MARKET</span></div><div class="subscriptionPlansGrid">${plans.map(p=>`<article class="subscriptionPlanCard ${p.code==='BUSINESS'?'premiumPlan':''}"><div class="planTopLine"><span>${esc(p.tag)}</span><b>${esc(p.status)}</b></div><h2>${esc(p.name)}</h2><div class="planPrice">${money(p.price)} <small>/ ${esc(p.duration)}</small></div><ul>${p.features.map(f=>`<li>✅ ${esc(f)}</li>`).join('')}</ul>${p.code==='FREE'?`<button class="freePlanBtn" disabled>Activé automatiquement pendant 10 jours</button>`:`<button class="buyPlanBtn" onclick="openSubscriptionPayment('${p.code}')">Choisir ${esc(p.name.replace('PLAN ',''))} — ${money(p.price)}/${p.code==='BUSINESS'?'an':'mois'}</button>`}</article>`).join('')}</div></div>`;
}

function openSubscriptionPayment(code){
  if(code==='FREE'){alert('Le Plan Free est activé automatiquement pendant 10 jours à la création de l’entreprise.');return;}
  if(!['STANDARD','BUSINESS'].includes(code)) return;
...
function marketplaceWhatsappNumbers(company){
  const source=Array.isArray(company?.marketWhatsappNumbers)?company.marketWhatsappNumbers:(company?.marketWhatsapp?String(company.marketWhatsapp).split(/[\n,;]+/):[]);
  return [...new Set(source.map(normalizeMarketplaceWhatsappNumber).filter(v=>/^\d{8,15}$/.test(v)))].slice(0,5);
}
function openMarketplaceWhatsappConfig(){
  const {company}=current();if(!company)return;
  document.getElementById('marketWhatsappConfigModal')?.remove();
  const numbers=marketplaceWhatsappNumbers(company);
  document.body.insertAdjacentHTML('beforeend',`<div class="marketPayModalBackdrop" id="marketWhatsappConfigModal"><div class="marketPayModal marketWhatsappConfigBox"><button class="marketPayClose" onclick="document.getElementById('marketWhatsappConfigModal')?.remove()">×</button><h2>WhatsApp des commandes</h2><p>Enregistrez le ou les numéros qui doivent recevoir les nouvelles commandes de cette boutique. Maximum : 5 numéros.</p><label>Numéro(s) WhatsApp<textarea id="marketWhatsappNumbers" rows="6" placeholder="Ex : 2250777041790\nUn numéro par ligne">${esc(numbers.join('\n'))}</textarea></label><small>Format conseillé : indicatif pays + numéro, sans espace. Exemple Côte d’Ivoire : 22507XXXXXXXX.</small><div class="marketPayActions"><button onclick="saveMarketplaceWhatsappConfig()">Enregistrer</button><button class="btn2" onclick="document.getElementById('marketWhatsappConfigModal')?.remove()">Fermer</button></div></div></div>`);
}
async function saveMarketplaceWhatsappConfig(){
  const {d,company,user}=current();if(!company||user?.role!=='admin')return alert('Accès réservé à l’administrateur de la boutique.');
  const values=String(document.getElementById('marketWhatsappNumbers')?.value||'').split(/[\n,;]+/).map(normalizeMarketplaceWhatsappNumber).filter(Boolean);
  const numbers=[...new Set(values.filter(v=>/^\d{8,15}$/.test(v)))].slice(0,5);
  if(values.length&&!numbers.length)return alert('Aucun numéro WhatsApp valide. Utilisez le format indicatif pays + numéro.');
  const target=(d.companies||[]).find(c=>String(c.id)===String(company.id));if(!target)return alert('Boutique introuvable.');
  target.marketWhatsappNumbers=numbers;
  target.marketWhatsapp=numbers[0]||'';
  target.marketWhatsappUpdatedAt=new Date().toISOString();
  const saveBtn=document.querySelector('#marketWhatsappConfigModal .marketPayActions button');if(saveBtn){saveBtn.disabled=true;saveBtn.textContent='Enregistrement…';}
  try{save(d);const ack=await cloudSaveNow(d);if(ack?.queued)throw new Error('Sauvegarde en attente de confirmation serveur.');document.getElementById('marketWhatsappConfigModal')?.remove();alert(numbers.length?'Numéro(s) WhatsApp des commandes enregistré(s).':'Numéros WhatsApp supprimés.');showMarketplacePage();}
  catch(error){if(saveBtn?.isConnected){saveBtn.disabled=false;saveBtn.textContent='Enregistrer';}alert(secureErrorMessage(error,'Enregistrement WhatsApp impossible.'));}
}
...
function showMarketplacePage(){if(isCaisse()) return alert('Accès interdit : la caisse ne peut pas administrer la marketplace.');
  ...
  if(!assertPlanFeature(company,'marketplace','La Marketplace est réservée aux Plans Standard et Business. Le Plan Free ne publie aucun produit ou service dans la boutique publique.')) return renderDash('home');
  const cid=company.id;
...
function legacyDup20_getPublicClient(companyId){
...
function legacyDup24_getPublicCart(companyId){
...
function legacyDup26_publicCartCount(companyId){return getPublicCart(companyId).reduce((a,x)=>a+Number(x.qty||0),0)}
...
async function legacyDup29_addToPublicCart(companyId,itemId){
...
function legacyDup28_refreshPublicCartBadge(companyId){const b=document.getElementById('publicCartBadge'); if(b) b.textContent=publicCartCount(companyId);}
...
function legacyDup34_openPublicCart(companyId){
...
function legacyDup31_updatePublicCartQty(companyId,itemId,qty){const cart=getPublicCart(companyId); const line=cart.find(x=>x.itemId===itemId); if(line) line.qty=Math.max(1,Number(qty||1)); openPublicCart(companyId); refreshPublicCartBadge(companyId);}
function legacyDup32_removePublicCartItem(companyId,itemId){const cart=getPublicCart(companyId); const i=cart.findIndex(x=>x.itemId===itemId); if(i>=0) cart.splice(i,1); openPublicCart(companyId); refreshPublicCartBadge(companyId);}
function legacyDup33_publicCartTotal(companyId){
...
async function legacyDup18_finalizePublicCartPayment(companyId,method){
...
function legacyDup41_openClientRegisterPopup(companyId){
...
function legacyDup40_openClientLoginPopup(companyId){
...
async function legacyDup16_savePublicClientRegister(companyId){
...
async function legacyDup17_loginPublicClient(companyId){
  ...
  window.publicShopClientId=client.id; document.getElementById('clientLoginModal')?.remove(); location.hash='#home'; renderGlobalShop();
}
function legacyDup45_openClientSpace(companyId){
  document.getElementById('clientSpaceModal')?.remove();
...
function restoreMarketplaceOrderStock(d,o){
  if(!d||!o||o.stockRestored)return [];
  const changed=[];
  for(const line of normalizeOrderItems(o)){
    const item=(d.items||[]).find(x=>String(x.id)===String(line.itemId)&&String(x.companyId)===String(o.companyId));
    if(!item)continue;
    const product=!['service','services','prestation'].includes(String(item.type||line.type||'').toLowerCase());
    if(product&&item.stockType!=='unlimited'){
...
function removeMarketplaceOrderFromReport(d,o){
  if(!d||!o)return;
  d.sales=(d.sales||[]).filter(s=>String(s.marketplaceOrderId||'')!==String(o.id||''));
  o.marketplaceReported=false;
  o.reportSaleIds=[];
}
function addMarketplaceOrderToReport(d,o){
  if(!d||!o)return;
  const existing=(d.sales||[]).filter(s=>String(s.marketplaceOrderId||'')===String(o.id||''));
  if(existing.length){o.marketplaceReported=true;o.reportSaleIds=existing.map(s=>s.id);return;}
  d.sales=d.sales||[];
  const now=o.paymentConfirmedAt||new Date().toISOString();
  const ids=[];
  for(const line of normalizeOrderItems(o)){
    const item=(d.items||[]).find(x=>String(x.id)===String(line.itemId)&&String(x.companyId)===String(o.companyId))||{};
    const qty=Math.max(1,Number(line.qty||1)),unit=Number(line.unit||0),total=Number(line.total||unit*qty);
    const product=!['service','services','prestation'].includes(String(item.type||line.type||'').toLowerCase());
    const charges=product?Number(item.buy||0)*qty:total*(Number(item.charge||0)/100);
    const sid=id('sale');ids.push(sid);
    d.sales.push({id:sid,companyId:o.companyId,userId:'marketplace',client:o.client||'Client marketplace',name:line.item||item.name||'Produit / service',qty,clientsServed:1,unit,total,serviceFee:Number(item.serviceFee||0),serviceBasePrice:Number(item.serviceBase||0),charges,profit:total-charges,date:now,cartCreatedAt:o.date||now,saleStatus:'validated',status:'validated',cartPending:false,validatedAt:now,clientType:'marketplace',itemCode:item.code||'',itemId:line.itemId||item.id||'',category:line.category||item.cat||'',detail:item.detail||item.marketplaceDesc||'',saleKind:product?'boutique':'service',note:'Commande GLOBAL MARKET #'+String(o.id||''),marketplaceOrderId:o.id,checkoutId:o.checkoutId||''});
  }
  o.marketplaceReported=true;
  o.reportSaleIds=ids;
}
async function saveMarketplaceOrderStatus(orderId){
  ...
  const saveBtn=document.querySelector('#marketOrderDetailsModal .marketPayActions button');
  if(saveBtn){saveBtn.disabled=true;saveBtn.textContent='Enregistrement…';}
  try{
    save(d);
    const ack=await cloudSaveNow(d);
    if(ack?.queued) throw new Error('La sauvegarde est en attente de confirmation du serveur.');
    document.getElementById('marketOrderDetailsModal')?.remove();
    showMarketplacePage();
    alert('Détails de la commande enregistrés.');
  }catch(error){
    alert(secureErrorMessage(error,'Enregistrement de la commande impossible.'));
    if(saveBtn?.isConnected){saveBtn.disabled=false;saveBtn.textContent='Enregistrer les détails';}
  }
}
...
function globalMarketCardHtml(i,company={}){
  ...
  const visual=i.photo?`<img src="${esc(i.photo)}" alt="${esc(i.name||'Produit ou service')}" loading="lazy">`:`<div class="gmHomeCardEmoji">${mkProductVisual(i)}</div>`;
  ...
  const actions=`<button type="button" class="gmHomeGhostBtn" onclick="event.stopPropagation();location.hash='#boutique/${shopSlug}';render()">Voir la boutique</button><button type="button" class="gmHomeCartBtn" onclick="event.stopPropagation();addGlobalMarketItemToCart('${esc(cid)}','${esc(iid)}')">${officialShopIcon('cart')} Ajouter au panier</button>`;
...
function renderGlobalMarketAbout(){
  app.innerHTML=`<main class="gmAboutPage gmHomePage gmAboutProPage">
    ${globalUniversalHeader('about')}

    <section class="gmAboutProHero">
      <div class="gmAboutProHeroCopy">
        <span class="gmAboutKicker">GLOBAL MARKET · SOLUTION DE GESTION & MARKETPLACE</span>
        <h1>Gérez votre activité.<br><em>Développez votre boutique en ligne.</em></h1>
        <p class="gmAboutProLead">GLOBAL MARKET réunit dans une seule plateforme les outils essentiels pour gérer une entreprise, présenter ses produits et services, recevoir des commandes et offrir aux clients un parcours d’achat professionnel.</p>
        <div class="gmAboutHeroActions"><button type="button" class="gmHomeCta" onclick="openGlobalBusinessAuth('register')">Créer ma boutique ${officialShopIcon('arrow')}</button><button type="button" class="gmAboutGhostBtn" onclick="goGlobalMarketHome()">Découvrir la marketplace</button></div>
        <div class="gmAboutTrustRow"><span>${officialShopIcon('shield')} Plateforme structurée</span><span>${officialShopIcon('cart')} Commandes suivies</span><span>${officialShopIcon('support')} Relation client facilitée</span></div>
      </div>
      <div class="gmAboutProHeroPanel">
        <div class="gmAboutProDashboardMark">${globalMarketBrandMark()}<div><b>GLOBAL MARKET</b><small>Votre activité, au même endroit</small></div></div>
        <div class="gmAboutProMetricGrid"><article><span>${officialShopIcon('computer')}</span><div><b>Gestion</b><small>Produits, ventes, stocks, clients et rapports</small></div></article><article><span>${officialShopIcon('bag')}</span><div><b>Boutique</b><small>Catalogue public et présentation professionnelle</small></div></article><article><span>${officialShopIcon('cart')}</span><div><b>Commandes</b><small>Suivi par boutique, paiement et livraison</small></div></article><article><span>${officialShopIcon('support')}</span><div><b>Messages</b><small>Échanges directs entre clients et vendeurs</small></div></article></div>
      </div>
    </section>

    <section class="gmAboutValueStrip" aria-label="Les piliers de GLOBAL MARKET"><article><b>01</b><span>Centraliser la gestion</span></article><article><b>02</b><span>Vendre en ligne</span></article><article><b>03</b><span>Suivre les commandes</span></article><article><b>04</b><span>Développer la relation client</span></article></section>

    <section class="gmAboutProSection">
      <div class="gmAboutProSectionHead"><span>01</span><div><small>PRÉSENTATION</small><h2>Une plateforme conçue pour gérer et vendre</h2><p>GLOBAL MARKET associe un espace de gestion interne à une marketplace publique afin d’éviter de multiplier les outils.</p></div></div>
      <div class="gmAboutProFeatureGrid">
        <article><div class="gmAboutIcon">${officialShopIcon('computer')}</div><h3>Gestion complète de l’activité</h3><p>Organisez produits et services, stocks, ventes, clients, commandes, mouvements et rapports depuis un espace professionnel unique.</p><ul><li>Catalogue et stock</li><li>Ventes et suivi</li><li>Rapports d’activité</li></ul></article>
        <article><div class="gmAboutIcon">${officialShopIcon('phone')}</div><h3>Boutique en ligne intégrée</h3><p>Présentez votre entreprise et publiez vos offres directement dans la marketplace GLOBAL MARKET.</p><ul><li>Boutique publique</li><li>Produits et services</li><li>Accès depuis ordinateur et téléphone</li></ul></article>
        <article><div class="gmAboutIcon">${officialShopIcon('cart')}</div><h3>Parcours commande maîtrisé</h3><p>Chaque commande est rattachée à la boutique concernée avec validation, paiement, livraison et historique côté client.</p><ul><li>Lots par boutique</li><li>Paiement encadré</li><li>Statuts de suivi</li></ul></article>
      </div>
    </section>

    <section class="gmAboutProSection gmAboutProMission">
      <div class="gmAboutProSectionHead"><span>02</span><div><small>NOTRE MISSION</small><h2>Rendre le commerce numérique plus simple et plus professionnel</h2><p>GLOBAL MARKET accompagne les acteurs économiques dans l’organisation de leur activité et leur présence en ligne.</p></div></div>
      <div class="gmAboutMissionGrid"><article class="gmAboutMiniCard"><strong>Organiser</strong><p>Regrouper les informations essentielles de l’entreprise dans un environnement structuré.</p></article><article class="gmAboutMiniCard"><strong>Valoriser</strong><p>Donner aux boutiques et prestataires une vitrine claire pour présenter leurs offres.</p></article><article class="gmAboutMiniCard"><strong>Connecter</strong><p>Faciliter la relation entre vendeurs et clients grâce aux commandes et à la messagerie.</p></article><article class="gmAboutMiniCard"><strong>Accompagner</strong><p>Permettre un suivi plus rigoureux des ventes, paiements et livraisons.</p></article></div>
    </section>

    <section class="gmAboutProSection">
      <div class="gmAboutProSectionHead"><span>03</span><div><small>POUR QUI ?</small><h2>Une solution ouverte à plusieurs profils</h2><p>GLOBAL MARKET s’adapte aux activités de vente de produits comme aux prestations de services.</p></div></div>
      <div class="gmAboutAudienceGrid"><article class="gmAboutAudienceCard"><div class="gmAboutIcon">${officialShopIcon('bag')}</div><h3>Boutiques & commerçants</h3><p>Publier des articles, gérer le stock et traiter les commandes reçues.</p></article><article class="gmAboutAudienceCard"><div class="gmAboutIcon">${officialShopIcon('spark')}</div><h3>Prestataires de services</h3><p>Présenter leurs prestations, enregistrer les ventes et mieux suivre les clients.</p></article><article class="gmAboutAudienceCard"><div class="gmAboutIcon">${officialShopIcon('computer')}</div><h3>Entreprises & PME</h3><p>Centraliser la gestion commerciale et disposer d’un espace boutique officiel.</p></article><article class="gmAboutAudienceCard"><div class="gmAboutIcon">${officialShopIcon('user')}</div><h3>Clients</h3><p>Commander, payer, suivre la livraison et consulter leur historique personnel.</p></article></div>
    </section>

    <section class="gmAboutProSection gmAboutGuideSection">
      <div class="gmAboutProSectionHead"><span>04</span><div><small>GUIDE D’UTILISATION</small><h2>Créer votre boutique en 5 étapes</h2><p>Un parcours simple pour commencer à publier et gérer votre activité sur GLOBAL MARKET.</p></div></div>
      <div class="gmAboutSteps gmAboutProSteps"><article><span>01</span><h3>Créer ma boutique</h3><p>Depuis l’accueil, cliquez sur « Créer ma boutique » pour ouvrir le formulaire d’inscription.</p></article><article><span>02</span><h3>Renseigner l’entreprise</h3><p>Complétez les informations de votre activité, du responsable et vos coordonnées.</p></article><article><span>03</span><h3>Accéder à l’espace boutique</h3><p>Connectez-vous avec votre identifiant administrateur pour ouvrir votre tableau de bord.</p></article><article><span>04</span><h3>Publier vos offres</h3><p>Ajoutez vos catégories, produits ou services puis rendez-les visibles dans votre boutique.</p></article><article><span>05</span><h3>Traiter les demandes</h3><p>Suivez vos commandes et messages, puis mettez à jour paiement et livraison.</p></article></div>
      <div class="gmAboutFinalCta"><div><small>PRÊT À COMMENCER ?</small><h2>Créez votre présence professionnelle sur GLOBAL MARKET.</h2><p>Gérez votre activité et rendez vos produits ou services accessibles à vos clients depuis un même espace.</p></div><button type="button" onclick="openGlobalBusinessAuth('register')">Créer ma boutique ${officialShopIcon('arrow')}</button></div>
    </section>
...
function legacyDup25_globalMarketCartCount(){window.publicShopCart=window.publicShopCart||{};return Object.values(window.publicShopCart).reduce((sum,cart)=>sum+(Array.isArray(cart)?cart.reduce((a,x)=>a+Number(x.qty||0),0):0),0)}
function legacyDup27_refreshGlobalMarketCartBadge(){const badge=document.getElementById('gmGlobalCartBadge');if(badge)badge.textContent=globalMarketCartCount();}
async function legacyDup30_addGlobalMarketItemToCart(companyId,itemId){await addToPublicCart(companyId,itemId);refreshGlobalMarketCartBadge();}
function legacyDup35_openGlobalMarketCart(){
...
function legacyDup37_openGlobalClientAuth(mode='login'){
...
function legacyDup38_submitGlobalClientLogin(){const cid=document.getElementById('gmClientCompany')?.value;if(!cid)return alert('Veuillez choisir une boutique.');loginPublicClient(cid);}
function legacyDup39_submitGlobalClientRegister(){const cid=document.getElementById('gmClientCompany')?.value;if(!cid)return alert('Veuillez choisir une boutique.');savePublicClientRegister(cid);}
...
function stockQuickAddCategoryFromFullForm(){
  ...
  const p=planDef(company); if(!canAddPlanCategory(d,company)) return g3Alert(`Limite atteinte : ${p.label} autorise ${p.maxCategories} catégorie(s) maximum. Passez à une formule supérieure pour en créer davantage.`,'Limite abonnement','warn');
...
async function addItem(){
  ...
  const planCheck=canAddPlanItem(d,company,cat,eid); if(!planCheck.ok) return g3Alert(planCheck.reason+' Passez à une formule supérieure pour augmenter cette limite.','Limite abonnement','warn');
...
function saveStockCategoryFromPopup(){
  ...
  if(!old){const p=planDef(company); if(!canAddPlanCategory(d,company)) return g3Alert(`Limite atteinte : ${p.label} autorise ${p.maxCategories} catégorie(s) maximum. Passez à une formule supérieure pour en créer davantage.`,'Limite abonnement','warn');}
  if(old){
...
async function login(){
  ...
  try{
    ...
    location.hash='#app';
    render();
    setTimeout(()=>gmPollRealtimeNotifications(),350);
  }catch(e){
    console.error(e);
...
async function legacyDup42_savePublicClientRegister(companyId){
...
async function legacyDup43_loginPublicClient(companyId){
  ...
  try{
    ...
    const j=await readApiPayload(r); PUBLIC_CLIENT_SESSION=j.session||null; window.publicShopClientId=j.client?.id||'';
    await cloudLoadPublicData(); document.getElementById('clientLoginModal')?.remove(); location.hash='#home'; renderGlobalShop();
  }catch(e){alert(secureErrorMessage(e,'Téléphone ou mot de passe incorrect.'))}
}
async function legacyDup44_logoutPublicClient(companyId){
...
async function legacyDup36_finalizePublicCartPayment(companyId,method){
...
async function legacyDup46_deleteMarketplaceOrder(orderId,isAdmin){
...
function globalUniversalHeader(active='public',options={}){
  ...
  if(user){
    if(user.role==='superadmin'){
      ...
      const adminBase=['home','vente','rapports','contrats','stocks','mois','param'];
      const adminOrder=hasPlanFeature(company,'marketplace')?[...adminBase,'marketplace']:adminBase;
    ...
    nav=home+(restricted?'':`<button type="button" class="${active==='client-orders'?'active':''}" onclick="openClientSpace()">${officialShopIcon('cart')}<span>Mes commandes</span></button><button type="button" class="${active==='client-messages'?'active':''}" onclick="openClientMessagesPopup()">${officialShopIcon('support')}<span>Messages</span></button><button type="button" class="${active==='client-cart'?'active':''}" onclick="openGlobalMarketCart()">${officialShopIcon('cart')}<span>Mon panier</span></button><button type="button" class="${active==='client-account'?'active':''}" onclick="openClientAccountInSpace()">${officialShopIcon('user')}<span>Mon compte</span></button>`);
    actions=`<div class="gmUniversalUser"><span class="gmUniversalIdentity">👤 <b>Compte client</b><small>Session sécurisée</small></span><button type="button" class="gmUniversalLogout" onclick="logoutPublicClient()">Déconnexion</button></div>`;
  }else{
    nav=home;
...
function legacyDup47_globalCartPricing(){
...
function legacyDup48_openGlobalMarketCart(){
  document.getElementById('gmGlobalCartModal')?.remove();
  const client=currentGlobalClient();
...
async function legacyDup50_finalizePublicCartPayment(companyId,method){return finalizeGlobalMarketCart();}
...
async function legacyDup52_savePublicClientRegister(){
  ...
  if(!payload.name||!payload.phone||!payload.password)return alert('Nom, téléphone et mot de passe obligatoires.');
  try{const r=await fetchWithTimeout('/api/public/client/register',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)},12000);const j=await readApiPayload(r);PUBLIC_CLIENT_SESSION=j.session||null;window.publicShopClientId=j.client?.id||'';await cloudLoadPublicData();document.getElementById('clientRegisterModal')?.remove();location.hash='#home';renderGlobalShop();setTimeout(()=>{gmPollRealtimeNotifications();},0);}catch(e){alert(secureErrorMessage(e,'Inscription client impossible.'));}
}
async function legacyDup51_loginPublicClient(){
  const phone=($('#clientLoginPhone')?.value||'').trim(),password=($('#clientLoginPass')?.value||'').trim();
  try{const r=await fetchWithTimeout('/api/public/client/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({phone,password})},12000);const j=await readApiPayload(r);PUBLIC_CLIENT_SESSION=j.session||null;window.publicShopClientId=j.client?.id||'';await cloudLoadPublicData();document.getElementById('clientLoginModal')?.remove();location.hash='#home';renderGlobalShop();setTimeout(()=>{gmPollRealtimeNotifications();},0);}catch(e){alert(secureErrorMessage(e,'Téléphone ou mot de passe incorrect.'));}
}
...
function gmClientOrderStatusBadge(o){
  if(gmOrderCancelled(o)) return '<span class="gmClientOrderStatus gmClientOrderStatusCancelled">Annulée</span>';
  if(gmOrderDelivered(o)) return '<span class="gmClientOrderStatus gmClientOrderStatusDelivered">Livrée</span>';
  if(gmOrderPaymentConfirmed(o)) return '<span class="gmClientOrderStatus gmClientOrderStatusShipping">Livraison en cours</span>';
  if(gmOrderPaymentDeclared(o)) return '<span class="gmClientOrderStatus gmClientOrderStatusPayment">Paiement en attente</span>';
  if(gmOrderValidated(o)) return '<span class="gmClientOrderStatus gmClientOrderStatusReady">À régler</span>';
  return '<span class="gmClientOrderStatus gmClientOrderStatusWaiting">En attente</span>';
}
function gmClientOrderShortId(o){
  const raw=String(o?.id||'CMD').replace(/^cmd_/i,'');
  return raw.length>14?raw.slice(0,8).toUpperCase():raw.toUpperCase();
}
function gmClientOrderCard(o,shop){
  const canDelete=gmOrderCanClientDelete(o);
  const clientCancelled=Boolean(o.clientCancelled);
  const delivered=gmOrderDelivered(o);
  const deleteArea=delivered?`<button type="button" class="gmOrderDeleteBtn" onclick="deleteMarketplaceOrder('${esc(o.id)}',false)">Supprimer</button>`:(!canDelete?'<span class="gmOrderLocked">Paiement confirmé</span>':(clientCancelled||gmOrderCancelled(o)?`<button type="button" class="gmOrderDeleteBtn" onclick="deleteMarketplaceOrder('${esc(o.id)}',false)">Supprimer</button>`:`<button type="button" class="gmOrderCancelBtn" onclick="cancelClientMarketplaceOrder('${esc(o.id)}')">Annuler</button>`));
  const payAction=gmOrderValidated(o)&&!gmOrderPaymentDeclared(o)&&!gmOrderPaymentConfirmed(o)&&!gmOrderCancelled(o)?`<button type="button" class="gmOrderStateBtn gmOrderStatePay" onclick="openClientOrderPayment('${esc(o.id)}')">Régler</button>`:'';
  const date=new Date(o.date),validDate=!Number.isNaN(date.getTime());
  const dateLabel=validDate?date.toLocaleDateString('fr-FR',{day:'2-digit',month:'2-digit',year:'numeric'}):'-';
  const timeLabel=validDate?date.toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit'}):'';
  return `<tr class="gmClientOrderRow"><td data-label="Commande"><div class="gmClientOrderRef"><b>#${esc(gmClientOrderShortId(o))}</b><small>${orderItemsCount(o)} article(s)</small></div></td><td data-label="Date"><div class="gmClientOrderDate"><b>${esc(dateLabel)}</b><small>${esc(timeLabel)}</small></div></td><td data-label="Statut">${gmClientOrderStatusBadge(o)}</td><td data-label="Total" class="gmClientOrderAmount"><b>${money(orderTotal(o))}</b></td><td data-label="Actions" class="gmClientOrderActionsCell"><div class="gmClientOrderLineActions">${payAction}<button type="button" class="gmOrderDetailsBtn" onclick="openMarketplaceOrderPopup('${esc(o.id)}',false)">Voir les détails</button>${deleteArea}</div></td></tr>`;
}
function openClientSpace(){
  if(!currentGlobalClient())return openGlobalClientAuth('login');
  location.hash='#client-commandes';
  renderClientOrdersPage();
}
function renderClientOrdersPage(){
  const d=seed(),client=currentGlobalClient();if(!client){location.hash='#home';return renderGlobalShop()}
  const companyMap=new Map((d.companies||[]).map(c=>[String(c.id),c]));
  const clientDeletedSet=new Set([...(client.deletedOrderIds||[]),...((d.clientDeletedOrders&&d.clientDeletedOrders[client.id])||[])]);
  const orders=(d.orders||[]).filter(o=>String(o.clientId)===String(client.id)&&!clientDeletedSet.has(String(o.id))).sort((a,b)=>new Date(b.date)-new Date(a.date));
  const grouped=new Map();
  for(const o of orders){const cid=String(o.companyId||'');if(!grouped.has(cid))grouped.set(cid,[]);grouped.get(cid).push(o)}
  const groupsHtml=[...grouped.entries()].map(([cid,list])=>{const shop=list[0]?.shopName||companyMap.get(cid)?.name||'Boutique';return `<section class="gmClientOrderShopGroup"><div class="gmClientOrderShopHead"><div>${officialShopIcon('bag')}<span><b>${esc(shop)}</b><small>${list.length} commande(s)</small></span></div></div><div class="gmClientOrderTableWrap"><table class="gmClientOrdersTable"><thead><tr><th>Commande</th><th>Date</th><th>Statut</th><th>Total</th><th class="gmClientOrderActionsHead">Actions</th></tr></thead><tbody>${list.map(o=>gmClientOrderCard(o,shop)).join('')}</tbody></table></div></section>`}).join('')||'<div class="gmClientOrdersEmpty">Aucune commande pour le moment.</div>';
  app.innerHTML=`<main class="gmClientPageShell">${globalUniversalHeader('client-orders')}<section class="gmClientPageContent gmClientOrdersWorkspace"><div class="gmClientSpaceHead"><div><span class="gmHomeModalKicker">COMPTE CLIENT GLOBAL MARKET</span><h2>Mes commandes</h2></div></div><div class="gmClientOrdersTitle"><div><p>Chaque lot est traité par la boutique concernée. Le paiement devient disponible après validation du vendeur.</p></div><span>${orders.length} commande(s)</span></div><div class="gmClientOrdersScroll">${groupsHtml}</div></section></main>`;
  requestAnimationFrame(()=>{refreshGlobalMarketCartBadge();initFlexibleHorizontalMenu();window.scrollTo({top:0,behavior:'smooth'})});
}
async function cancelClientMarketplaceOrder(orderId){
  ...
  try{await securePublicPost('/api/public/order/action',{orderId,action:'cancel'});await gmRefreshPublicDataAfterAction();document.getElementById('clientSpaceModal')?.remove();openClientSpace();}catch(e){alert(secureErrorMessage(e,'Annulation impossible.'))}
}
async function deleteMarketplaceOrder(orderId,isAdmin){
  ...
  const d=seed(),o=(d.orders||[]).find(x=>String(x.id)===String(orderId));if(!o)return alert('Commande introuvable.');
  if(gmOrderDelivered(o)){
    if(!(await g3Confirm('Supprimer cette commande livrée de votre historique ? La boutique et l’administration conserveront leur trace de traitement.','Supprimer de mon historique')))return;
    try{await securePublicPost('/api/public/order/delete',{orderId});await gmRefreshPublicDataAfterAction();document.getElementById('marketOrderDetailsModal')?.remove();document.getElementById('clientSpaceModal')?.remove();openClientSpace();}catch(e){alert(secureErrorMessage(e,'Suppression impossible.'))}
    return;
  }
...
function showClientOrderPaymentMethod(orderId,method){
  ...
  const target=document.getElementById('gmOrderPaymentChoice');if(target)target.innerHTML=paymentBox+`<div class="gmOrderTransactionForm"><label>ID de la transaction<input id="gmOrderTransactionId" placeholder="Saisissez l’identifiant de la transaction"></label><button type="button" onclick="submitClientOrderPayment('${esc(o.id)}','${esc(method)}',this)">J’ai payé</button><small class="gmPaymentWhatsappHint">Après le clic, votre propre WhatsApp s’ouvrira directement sur la boutique vendeuse avec le message de paiement et les informations de la commande déjà préparés. Il vous restera à appuyer sur Envoyer dans WhatsApp.</small></div>`;
}
function clientOrderPaymentWhatsappMessage(order,company,method,transactionId){
  const items=normalizeOrderItems(order).slice(0,20).map((line,index)=>`${index+1}. ${String(line.item||'Article')} x${Number(line.qty||1)} — ${money(Number(line.total||0))}`);
  const delivery=[String(order?.deliveryCity||'').trim(),String(order?.deliveryNeighborhood||'').trim(),String(order?.deliveryAddressDetail||'').trim()].filter(Boolean).join(' — ')||'-';
  const subtotal=Number(order?.subtotal??(orderTotal(order)-Number(order?.deliveryFee||0)));
  return [
    'PAIEMENT DÉCLARÉ — GLOBAL MARKET CI',
    `Commande : #${String(order?.id||'')}`,
    `Boutique : ${String(order?.shopName||company?.name||'Boutique')}`,
    `Client : ${String(order?.client||'Client')}`,
    `Téléphone : ${String(order?.clientPhone||'')}`,
    '',
    'ARTICLES',
    ...items,
    '',
    `Sous-total : ${money(subtotal)}`,
    `Frais de livraison : ${money(Number(order?.deliveryFee||0))}`,
    `TOTAL : ${money(orderTotal(order))}`,
    `Paiement : ${String(method||order?.paymentMethod||'')}`,
    `ID transaction : ${String(transactionId||'')}`,
    `Livraison : ${delivery}`,
    `Mode de livraison : ${String(order?.shippingMethod||'-')}`,
    '',
    'Le client indique avoir effectué le paiement. Merci de vérifier la transaction dans GLOBAL MARKET CI avant de confirmer le paiement.'
  ].join('\n');
}
async function submitClientOrderPayment(orderId,method,button=null){
  ...
  // Ouvrir immédiatement une fenêtre depuis le clic du client évite que les navigateurs
  // bloquent WhatsApp après l'appel asynchrone au serveur. Le client reste l'expéditeur :
  // GLOBAL MARKET CI ne transmet aucun message WhatsApp côté serveur.
  let whatsappWindow=null;
  try{whatsappWindow=window.open('about:blank','_blank');if(whatsappWindow)whatsappWindow.opener=null;}catch{whatsappWindow=null;}
  const btn=button;if(btn){btn.disabled=true;btn.textContent='Ouverture de WhatsApp…';}
  try{
    const result=await securePublicPost('/api/public/order/action',{orderId,action:'declare_payment',paymentMethod:method,transactionId});
    const primaryWhatsapp=normalizeMarketplaceWhatsappNumber(result?.whatsappRecipient||'');
    if(!primaryWhatsapp){
      if(whatsappWindow&&!whatsappWindow.closed)whatsappWindow.close();
      if(btn?.isConnected){btn.disabled=false;btn.textContent='J’ai payé';}
      return alert('Le numéro WhatsApp de réception de cette boutique n’est pas encore configuré. Demandez à la boutique de renseigner Marketplace > WhatsApp commandes.');
    }

    const message=clientOrderPaymentWhatsappMessage(order,company,method,transactionId);
    const whatsappUrl=`https://wa.me/${encodeURIComponent(primaryWhatsapp)}?text=${encodeURIComponent(message)}`;
    if(whatsappWindow&&!whatsappWindow.closed)whatsappWindow.location.replace(whatsappUrl);else window.location.assign(whatsappUrl);

    // Rafraîchissement en arrière-plan : l'ouverture de WhatsApp reste l'action principale.
    gmRefreshPublicDataAfterAction().then(()=>{
      document.getElementById('gmClientOrderPaymentModal')?.remove();
      document.getElementById('clientSpaceModal')?.remove();
      openClientSpace();
    }).catch(()=>{});
    ...
    if(whatsappWindow&&!whatsappWindow.closed)whatsappWindow.close();
    if(btn?.isConnected){btn.disabled=false;btn.textContent='J’ai payé';}
    alert(secureErrorMessage(e,'Transmission du paiement impossible.'));
  }
}
...
async function sendBoutiqueMessage(companyId,threadId=''){
  ...
  try{const r=await fetchWithTimeout('/api/public/message',{method:'POST',headers:clientSecurityHeaders({'Content-Type':'application/json'}),body:JSON.stringify(payload),cache:'no-store'},12000);await readApiPayload(r);await gmRefreshPublicDataAfterAction();document.getElementById('gmBoutiqueContactModal')?.remove();alert('Votre message a été envoyé à la boutique.');if(currentGlobalClient())openClientMessagesPopup();}
...
async function deleteSelectedAdminMessages(){const ids=[...document.querySelectorAll('.gmAdminMsgCheck:checked')].map(x=>x.value);if(!ids.length)return alert('Sélectionnez au moins un message.');if(!(await g3Confirm(`Supprimer ${ids.length} message(s) de votre espace ?`,'Suppression messages')))return;const {d,company}=current();(d.marketMessages||[]).forEach(m=>{if(String(m.companyId)===String(company.id)&&ids.includes(String(m.id)))m.deletedByAdmin=true});try{save(d);const ack=await cloudSaveNow(d);if(ack?.queued)throw new Error('Suppression en attente de confirmation du serveur.');showMarketplaceAdminPage('messages')}catch(error){alert(secureErrorMessage(error,'Suppression des messages impossible.'));}}
async function deleteAllAdminMessages(){if(!(await g3Confirm('Supprimer tous les messages de votre espace Marketplace ?','Tout supprimer')))return;const {d,company}=current();(d.marketMessages||[]).forEach(m=>{if(String(m.companyId)===String(company.id))m.deletedByAdmin=true});try{save(d);const ack=await cloudSaveNow(d);if(ack?.queued)throw new Error('Suppression en attente de confirmation du serveur.');showMarketplaceAdminPage('messages')}catch(error){alert(secureErrorMessage(error,'Suppression des messages impossible.'));}}
...
async function sendAdminMessageReply(messageId){const {d,company,user}=current();d.marketMessages=d.marketMessages||[];const source=d.marketMessages.find(m=>String(m.id)===String(messageId));if(!source)return alert('Message introuvable.');const body=($('#gmAdminReplyBody')?.value||'').trim();if(!body)return alert('Rédigez votre réponse.');const row={id:id('msg'),companyId:company.id,clientId:source.clientId||'',threadId:gmMessageThreadId(source),senderType:'admin',senderName:company.name||user?.name||'Boutique',senderPhone:company.phone||'',senderEmail:company.email||'',subject:'RE: '+String(source.subject||'Demande client').replace(/^RE:\s*/i,''),body,createdAt:new Date().toISOString(),deletedByAdmin:false,deletedByClient:false};d.marketMessages.push(row);const btn=document.querySelector('#gmAdminMessageReplyModal .gmMessageFormActions button:last-child');if(btn){btn.disabled=true;btn.textContent='Envoi…';}try{save(d);const ack=await cloudSaveNow(d);if(ack?.queued)throw new Error('Réponse en attente de confirmation du serveur.');document.getElementById('gmAdminMessageReplyModal')?.remove();showMarketplaceAdminPage('messages');alert('Réponse enregistrée et visible dans l’espace client.');}catch(error){d.marketMessages=d.marketMessages.filter(m=>m!==row);alert(secureErrorMessage(error,'Envoi de la réponse impossible.'));if(btn?.isConnected){btn.disabled=false;btn.textContent='Envoyer la réponse';}}}


function openClientAccountInSpace(){
  if(!currentGlobalClient())return openGlobalClientAuth('login');
  location.hash='#client-compte';
  renderClientAccountPage();
}
function renderClientAccountPage(){
  const client=currentGlobalClient();if(!client){location.hash='#home';return renderGlobalShop()}
  const created=client.createdAt?new Date(client.createdAt).toLocaleString('fr-FR'):'Non disponible';
  app.innerHTML=`<main class="gmClientPageShell">${globalUniversalHeader('client-account')}<section class="gmClientPageContent gmClientAccountWorkspace"><div class="gmClientSpaceHead"><div><span class="gmHomeModalKicker">MON COMPTE CLIENT</span><h2>${esc(client.name||'Client')}</h2><p>Gérez vos informations personnelles et vos accès GLOBAL MARKET.</p></div></div><div class="gmClientAccountSummary"><article><small>N° compte client</small><b>${esc(client.id||'-')}</b></article><article><small>Statut du compte</small><b>Actif</b></article><article><small>Identifiant de connexion</small><b>${esc(client.phone||'-')}</b></article><article><small>Créé le</small><b>${esc(created)}</b></article></div><section class="gmClientAccountForm"><h3>Informations personnelles</h3><div class="gmClientAccountGrid"><label>Nom complet<input id="gmAccountClientName" value="${esc(client.name||'')}" autocomplete="name"></label><label>Email<input id="gmAccountClientEmail" type="email" value="${esc(client.email||'')}" autocomplete="email"></label><label>Identifiant de connexion (téléphone)<input id="gmAccountClientPhone" value="${esc(client.phone||'')}" inputmode="tel" autocomplete="tel"></label></div><h3>Sécurité du compte</h3><p class="sub">Le mot de passe actuel est obligatoire pour changer l’identifiant ou le mot de passe.</p><div class="gmClientAccountGrid"><label>Mot de passe actuel<input id="gmAccountCurrentPassword" type="password" autocomplete="current-password" placeholder="Votre mot de passe actuel"></label><label>Nouveau mot de passe<input id="gmAccountNewPassword" type="password" autocomplete="new-password" placeholder="Laisser vide pour conserver"></label><label>Confirmer le nouveau mot de passe<input id="gmAccountConfirmPassword" type="password" autocomplete="new-password" placeholder="Confirmer le nouveau mot de passe"></label></div><div class="gmClientAccountActions"><button type="button" class="gmHomePrimaryAuth" onclick="saveGlobalClientAccount()">Enregistrer les modifications</button></div></section></section></main>`;
  requestAnimationFrame(()=>{refreshGlobalMarketCartBadge();initFlexibleHorizontalMenu();window.scrollTo({top:0,behavior:'smooth'})});
}
async function saveGlobalClientAccount(){
  ...
  try{
    ...
    await gmRefreshPublicDataAfterAction();location.hash='#client-compte';renderClientAccountPage();alert('Votre compte client a été mis à jour.');
...
function openClientMessagesPopup(){
  if(!currentGlobalClient())return openGlobalClientAuth('login');
  location.hash='#client-messages';
  renderClientMessagesPage();
}
function renderClientMessagesPage(){
  const client=currentGlobalClient();if(!client){location.hash='#home';return renderGlobalShop()}
  const d=seed(),companies=d.companies||[],messages=gmClientVisibleMessages();
  const companyOptions=companies.filter(c=>hasPlanFeature(c,'public_shop')).map(c=>`<option value="${esc(c.id)}">${esc(c.name||'Boutique')}</option>`).join('');
...
function buildGlobalMarketCartPopupLegacy(){
  document.getElementById('gmGlobalCartModal')?.remove();
  ...
  document.body.insertAdjacentHTML('beforeend',`<div class="gmHomeAuthBackdrop" id="gmGlobalCartModal" onclick="if(event.target.id==='gmGlobalCartModal')this.remove()"><section class="gmHomeAuthCard gmUnifiedCartCard"><button class="gmHomeAuthClose" onclick="document.getElementById('gmGlobalCartModal')?.remove()">×</button><span class="gmHomeModalKicker">PANIER UNIQUE CLIENT</span><div class="gmUnifiedCartTitle"><div><h2>Mon panier GLOBAL MARKET</h2><p>Un seul panier pour vos produits et services de plusieurs boutiques.</p></div><span class="gmCartClientChip">${officialShopIcon('user')} Session client sécurisée</span></div><div class="gmUnifiedCartMain"><div class="gmUnifiedCartGroups">${groupsHtml}</div><div class="gmCartCheckoutGrid">${addressCard}${shippingCard}${summaryCard}</div></div><div class="gmUnifiedCartActions"><button class="btn2" onclick="clearGlobalClientCart();openGlobalMarketCart()">Vider le panier</button><button class="gmHomePrimaryAuth" onclick="finalizeGlobalMarketCart()" ${orderDisabled?'disabled':''}>Passer ma commande</button></div></section></div>`);
}
function openGlobalMarketCart(){
  if(!currentGlobalClient())return openGlobalClientAuth('login');
  location.hash='#client-panier';
  renderClientCartPage();
}
function renderClientCartPage(){
  const client=currentGlobalClient();if(!client){location.hash='#home';return renderGlobalShop()}
  const cart=getGlobalClientCart()||[];
  if(!cart.length){
    app.innerHTML=`<main class="gmClientPageShell">${globalUniversalHeader('client-cart')}<section class="gmClientPageContent gmClientCartPageContent"><div class="gmClientPageEmpty"><div>${officialShopIcon('cart')}</div><h2>Mon panier</h2><p>Votre panier est vide. Ajoutez des produits ou services depuis l’accueil ou les boutiques.</p><button type="button" class="gmHomePrimaryAuth" onclick="goGlobalMarketHome()">Découvrir les produits</button></div></section></main>`;
    requestAnimationFrame(()=>{refreshGlobalMarketCartBadge();initFlexibleHorizontalMenu();window.scrollTo({top:0,behavior:'smooth'})});
    return;
  }
  document.getElementById('gmGlobalCartModal')?.remove();
  buildGlobalMarketCartPopupLegacy();
  const modal=document.getElementById('gmGlobalCartModal');
  const card=modal?.querySelector('.gmUnifiedCartCard');
  if(!card)return;
  card.querySelector('.gmHomeAuthClose')?.remove();
  const html=card.innerHTML;
  modal.remove();
  app.innerHTML=`<main class="gmClientPageShell">${globalUniversalHeader('client-cart')}<section class="gmClientPageContent gmClientCartPageContent gmUnifiedCartCard">${html}</section></main>`;
  requestAnimationFrame(()=>{refreshGlobalMarketCartBadge();initFlexibleHorizontalMenu();window.scrollTo({top:0,behavior:'smooth'})});
}
async function finalizeGlobalMarketCart(){
  ...
  try{
    ...
    const r=await fetchWithTimeout('/api/public/order',{method:'POST',headers:clientSecurityHeaders({'Content-Type':'application/json'}),body:JSON.stringify(payload),cache:'no-store'},25000);
    const j=await readApiPayload(r);cart.length=0;window.GLOBAL_MARKET_CART_DELIVERY={city:'',neighborhood:'',addressDetail:'',methods:{}};refreshGlobalMarketCartBadge();await gmRefreshPublicDataAfterAction();document.getElementById('gmGlobalCartModal')?.remove();
...
async function loginPublicClient(){
  ...
  try{const r=await fetchWithTimeout('/api/public/client/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({phone,password})},12000);const j=await readApiPayload(r);PUBLIC_CLIENT_SESSION=j.session||null;window.publicShopClientId=j.client?.id||'';await cloudLoadPublicData();document.getElementById('clientLoginModal')?.remove();location.hash='#home';renderGlobalShop();setTimeout(()=>{gmPollRealtimeNotifications();},0);}catch(e){alert(secureErrorMessage(e,'Téléphone ou mot de passe incorrect.'));}finally{GM_CLIENT_LOGIN_BUSY=false;if(btn?.isConnected){btn.disabled=false;btn.textContent='Se connecter';}}
}
async function savePublicClientRegister(){
  ...
  try{const r=await fetchWithTimeout('/api/public/client/register',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)},12000);const j=await readApiPayload(r);PUBLIC_CLIENT_SESSION=j.session||null;window.publicShopClientId=j.client?.id||'';await cloudLoadPublicData();document.getElementById('clientRegisterModal')?.remove();location.hash='#home';renderGlobalShop();setTimeout(()=>{gmPollRealtimeNotifications();},0);}catch(e){alert(secureErrorMessage(e,'Inscription client impossible.'));}finally{GM_CLIENT_REGISTER_BUSY=false;if(btn?.isConnected){btn.disabled=false;btn.textContent='Créer mon compte client';}}
}
