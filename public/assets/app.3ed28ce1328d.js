...
/* ========================================================================
   GLOBAL MARKET V5.2 — CONFIGURATION LIVRAISON, PANIER & SECURITE CONNEXION
   ======================================================================== */
...
function gmMarketplaceDeliveryConfig(company){
  ...
  let cities=(Array.isArray(raw.cities)?raw.cities:GM_DEFAULT_DELIVERY_CITIES).slice(0,10).map(x=>({name:gmDeliveryToken(x?.name),fee:Math.max(0,Math.round(Number(x?.fee||0)))})).filter(x=>x.name);
  ...
  if(homeCity&&!seenCities.has(homeCity)){cities.unshift({name:homeCity,fee:0});cities=cities.slice(0,10);}
  let methods=(Array.isArray(raw.methods)&&raw.methods.length?raw.methods:GM_DEFAULT_SHIPPING_METHODS).slice(0,10).map(x=>({id:String(x?.id||gmMethodId(x?.name)).trim(),name:gmDeliveryToken(x?.name),fee:Math.max(0,Math.round(Number(x?.fee||0)))})).filter(x=>x.name);
  const seenMethods=new Set();methods=methods.filter(x=>{const t=String(x.id||gmMethodId(x.name));if(seenMethods.has(t))return false;seenMethods.add(t);x.id=t;return true;});
  if(!methods.length)methods=GM_DEFAULT_SHIPPING_METHODS.map(x=>({...x}));
...
function gmDeliveryCityFee(config,city,subtotal,method){
  const cityToken=gmDeliveryToken(city),local=cityToken===gmDeliveryToken(config.homeCity),pickup=gmIsPickupMethod(method);
  if(pickup&&local)return {cityFee:0,rate:0,local:true,pickup:true};
  if(local){const rate=gmLocalDeliveryRate(subtotal);return {cityFee:Math.round(Number(subtotal||0)*rate),rate,local:true,pickup:false};}
  const row=(config.cities||[]).find(x=>gmDeliveryToken(x.name)===cityToken);
  return {cityFee:Math.max(0,Math.round(Number(row?.fee||0))),rate:0,local:false,pickup:false};
}
function gmDeliveryConfigCityRow(city={},homeCity=''){
  ...
  return `<div class="gmDeliveryConfigRow gmDeliveryCityRow">
    ...
    <input class="gmDeliveryCityFee" type="number" min="0" step="50" placeholder="Frais ville (FCFA)" value="${local?'':Math.max(0,Number(city.fee||0))}" ${local?'disabled':''}>
    <span class="gmDeliveryLocalHint">${local?'Pourcentage local automatique':'Frais fixes de la ville'}</span>
    <button type="button" class="miniDanger" onclick="this.closest('.gmDeliveryCityRow')?.remove()">Retirer</button>
  </div>`;
}
function gmShippingMethodRow(method={}){
  return `<div class="gmDeliveryConfigRow gmShippingMethodRow">
    <input class="gmShippingMethodName" maxlength="60" placeholder="Moyen d’expédition" value="${esc(gmDeliveryToken(method.name||''))}">
    <input class="gmShippingMethodFee" type="number" min="0" step="50" placeholder="Frais du moyen (FCFA)" value="${Math.max(0,Number(method.fee||0))}">
    <span class="gmDeliveryLocalHint">Frais ajoutés selon le moyen choisi</span>
    <button type="button" class="miniDanger" onclick="this.closest('.gmShippingMethodRow')?.remove()">Retirer</button>
  </div>`;
}
function openMarketplaceDeliveryConfig(){
  ...
  const html=`<div class="marketPayModalBackdrop" id="marketDeliveryConfigModal"><div class="marketPayModal gmDeliveryConfigModal">
    ...
    <p class="gmDeliveryIntro">Chaque administrateur configure jusqu’à <b>10 villes de livraison</b>, y compris la ville de sa boutique, ainsi que ses moyens d’expédition et leurs frais.</p>
    <section class="gmDeliveryConfigSection">
      <div class="gmDeliveryConfigTitle"><div><h3>1. Ville de la boutique</h3><p>Pour les clients de cette ville, le frais de base est calculé automatiquement par pourcentage du sous-total.</p></div></div>
    ...
    <section class="gmDeliveryConfigSection">
      <div class="gmDeliveryConfigTitle"><div><h3>2. Villes de livraison</h3><p>La ville de la boutique doit obligatoirement figurer dans la liste. Pour les autres villes, définissez le frais fixe de destination.</p></div><button type="button" class="btn2" onclick="gmAddDeliveryCityRow()">+ Ajouter une ville</button></div>
    ...
    <section class="gmDeliveryConfigSection">
      <div class="gmDeliveryConfigTitle"><div><h3>3. Moyens d’expédition</h3><p>Le frais du moyen choisi s’ajoute automatiquement au frais de destination. « Retrait à la boutique » reste disponible uniquement dans la ville de la boutique.</p></div><button type="button" class="btn2" onclick="gmAddShippingMethodRow()">+ Ajouter un moyen</button></div>
...
function gmRefreshDeliveryConfigRows(){
  ...
  document.querySelectorAll('#gmDeliveryCitiesRows .gmDeliveryCityRow').forEach(row=>{
    ...
    const fee=row.querySelector('.gmDeliveryCityFee'),hint=row.querySelector('.gmDeliveryLocalHint');
    const local=!!home&&name===home;
    if(fee){fee.disabled=local;if(local)fee.value='';}
    if(hint)hint.textContent=local?'Pourcentage local automatique':'Frais fixes de la ville';
  });
}
async function saveMarketplaceDeliveryConfig(){
  ...
  const cityRows=[...document.querySelectorAll('#gmDeliveryCitiesRows .gmDeliveryCityRow')].map(row=>({name:gmDeliveryToken(row.querySelector('.gmDeliveryCityName')?.value||''),fee:Math.max(0,Math.round(Number(row.querySelector('.gmDeliveryCityFee')?.value||0)))})).filter(x=>x.name);
  const uniqueCities=[];const citySeen=new Set();
  for(const row of [{name:homeCity,fee:0},...cityRows]){const t=gmDeliveryToken(row.name);if(!t||citySeen.has(t))continue;citySeen.add(t);uniqueCities.push({name:t,fee:t===homeCity?0:row.fee});if(uniqueCities.length>=10)break;}
  const methodRows=[...document.querySelectorAll('#gmShippingMethodsRows .gmShippingMethodRow')].map(row=>({name:gmDeliveryToken(row.querySelector('.gmShippingMethodName')?.value||''),fee:Math.max(0,Math.round(Number(row.querySelector('.gmShippingMethodFee')?.value||0)))})).filter(x=>x.name);
  const uniqueMethods=[];const methodSeen=new Set();
  for(const row of methodRows){const id=gmMethodId(row.name);if(methodSeen.has(id))continue;methodSeen.add(id);uniqueMethods.push({id,name:row.name,fee:row.fee});if(uniqueMethods.length>=10)break;}
  if(!uniqueMethods.length)return alert('Ajoutez au moins un moyen d’expédition.');
  target.marketDeliveryConfig={homeCity,cities:uniqueCities,methods:uniqueMethods,updatedAt:new Date().toISOString()};
...
function gmCartDeliveryState(){
  window.GLOBAL_MARKET_CART_DELIVERY=window.GLOBAL_MARKET_CART_DELIVERY||{city:'',addressDetail:'',methods:{}};
...
function gmAvailableShippingMethods(company,city){
  ...
  return (cfg.methods||[]).filter(method=>local||!gmIsPickupMethod(method));
}
function gmSetCartDeliveryCity(value){const st=gmCartDeliveryState();st.city=gmDeliveryToken(value);st.methods={};openGlobalMarketCart();}
...
function gmCartShopPricing(companyId,rows,company,city){
  ...
  if(!selected)return {subtotal,cityFee:0,methodFee:0,deliveryFee:0,total:subtotal,method:null,rate:0,valid:false};
  const base=gmDeliveryCityFee(cfg,city,subtotal,selected),methodFee=Math.max(0,Math.round(Number(selected.fee||0)));
  const deliveryFee=base.cityFee+methodFee;
  return {subtotal,cityFee:base.cityFee,methodFee,deliveryFee,total:subtotal+deliveryFee,method:selected,rate:base.rate,valid:true,local:base.local,pickup:base.pickup};
}
function globalCartPricing(){
  ...
  return {subtotal,deliveryFee,total:subtotal+deliveryFee,shopCount:grouped.size,valid,city:st.city,cities};
}
function openGlobalMarketCart(){
  document.getElementById('gmGlobalCartModal')?.remove();
  ...
  const groupsHtml=[...grouped.entries()].map(([cid,rows])=>{
    ...
    const methodOptions=methods.map(m=>`<option value="${esc(m.id)}" ${pricing.method?.id===m.id?'selected':''}>${esc(m.name)} — ${money(m.fee)}</option>`).join('');
    const feeNote=pricing.pickup?'Retrait à la boutique':pricing.local?`${deliveryRateLabel(pricing.rate)} local + moyen`:`${money(pricing.cityFee)} destination + moyen`;
    return `<section class="gmUnifiedCartShop">
      ...
      <div class="gmUnifiedShippingRow"><label><span>Moyen d’expédition</span><select onchange="gmSetCartShippingMethod('${esc(cid)}',this.value)" ${methods.length?'':'disabled'}>${methodOptions||'<option>Aucun moyen disponible</option>'}</select></label><div><small>${esc(feeNote)}</small><span>Frais d’expédition <b>${money(pricing.deliveryFee)}</b></span></div></div>
    </section>`;
  }).join('');
  const requiresAddress=[...grouped.entries()].some(([cid,rows])=>{const p=gmCartShopPricing(cid,rows,companyMap.get(cid)||{},st.city);return !p.pickup;});
  const total=subtotalGeneral+feeGeneral;
  const deliveryPanel=cities.length?`<aside class="gmUnifiedDeliveryPanel"><h3>Adresse de livraison</h3><label>Ville de livraison *<select id="gmCartDeliveryCity" onchange="gmSetCartDeliveryCity(this.value)">${cityOptions}</select></label><label>Détail sur l’adresse de livraison ${requiresAddress?'*':''}<textarea id="gmCartDeliveryAddress" maxlength="500" placeholder="Quartier, rue, immeuble, porte, repère..." oninput="gmSetCartAddressDetail(this.value)">${esc(st.addressDetail||'')}</textarea></label><div class="gmUnifiedShippingAuto"><span>🚚</span><div><small>Frais d’expédition automatiques</small><b>${money(feeGeneral)}</b><em>Calculés selon la ville, la boutique et le moyen choisi.</em></div></div></aside>`:`<aside class="gmUnifiedDeliveryPanel gmDeliveryUnavailable"><h3>Adresse de livraison</h3><p>Aucune ville de livraison commune n’est configurée pour toutes les boutiques de ce panier.</p><small>Retirez un article d’une boutique ou demandez aux administrateurs d’ajouter une ville commune.</small></aside>`;
  const orderDisabled=!allMethodsValid||!cities.length;
  document.body.insertAdjacentHTML('beforeend',`<div class="gmHomeAuthBackdrop" id="gmGlobalCartModal" onclick="if(event.target.id==='gmGlobalCartModal')this.remove()"><section class="gmHomeAuthCard gmUnifiedCartCard"><button class="gmHomeAuthClose" onclick="document.getElementById('gmGlobalCartModal')?.remove()">×</button><span class="gmHomeModalKicker">PANIER UNIQUE CLIENT</span><div class="gmUnifiedCartTitle"><div><h2>Mon panier GLOBAL MARKET</h2><p>Un seul panier pour vos produits et services de plusieurs boutiques.</p></div><span class="gmCartClientChip">${officialShopIcon('user')} ${esc(client.name||'Client connecté')}</span></div><div class="gmUnifiedCartLayout"><div class="gmUnifiedCartMain"><div class="gmUnifiedCartGroups">${groupsHtml}</div><div class="gmUnifiedCartSummary"><div><span>Sous-total produits & services</span><b>${money(subtotalGeneral)}</b></div><div><span>Frais d’expédition</span><b>${money(feeGeneral)}</b></div><div class="grand"><span>Total général</span><b>${money(total)}</b></div></div></div>${deliveryPanel}</div><div class="gmUnifiedCartActions"><button class="btn2" onclick="clearGlobalClientCart();document.getElementById('gmGlobalCartModal')?.remove()">Vider le panier</button><button class="gmHomePrimaryAuth" onclick="finalizeGlobalMarketCart()" ${orderDisabled?'disabled':''}>Passer ma commande</button></div><p class="gmUnifiedCartPayment">Paiement appliqué : <b>Paiement à la livraison</b>.</p></section></div>`);
}
async function finalizeGlobalMarketCart(){
  ...
  const d=seed(),companyMap=new Map((d.companies||[]).map(c=>[c.id,c])),grouped=gmCartGroupedLines();
  const requiresAddress=[...grouped.entries()].some(([cid,rows])=>!gmCartShopPricing(cid,rows,companyMap.get(cid)||{},st.city).pickup);
  if(requiresAddress&&!String(st.addressDetail||'').trim())return alert('Renseignez le détail sur l’adresse de livraison.');
  const shippingByCompany=[...grouped.entries()].map(([cid,rows])=>{const p=gmCartShopPricing(cid,rows,companyMap.get(cid)||{},st.city);return {companyId:cid,methodId:p.method?.id||''};});
  const button=document.querySelector('.gmUnifiedCartActions .gmHomePrimaryAuth');if(button){button.disabled=true;button.textContent='Envoi de la commande…';}
  try{
    const payload={cart:cart.map(x=>({itemId:x.itemId,qty:Number(x.qty||1)})),paymentMethod:'PAIEMENT À LA LIVRAISON',deliveryCity:st.city,deliveryAddressDetail:String(st.addressDetail||'').trim(),shippingByCompany};
    const r=await fetchWithTimeout('/api/public/order',{method:'POST',headers:clientSecurityHeaders({'Content-Type':'application/json'}),body:JSON.stringify(payload),cache:'no-store'},25000);
    const j=await readApiPayload(r);cart.length=0;window.GLOBAL_MARKET_CART_DELIVERY={city:'',addressDetail:'',methods:{}};refreshGlobalMarketCartBadge();await cloudLoadPublicData();document.getElementById('gmGlobalCartModal')?.remove();
