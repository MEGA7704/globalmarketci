...
function gmDashboardHomeHTML(ctx={}){
  ...
  const quickActions=[
    {icon:'🛒',tone:'green',title:'Nouvelle vente',sub:'Enregistrer une vente',action:"show('vente')"},{icon:'▣',tone:'blue',title:'Nouvelle commande',sub:'Créer une commande',action:"show('vente')"},{icon:'👥',tone:'violet',title:'Ajouter un client',sub:'Nouveau client',action:"show('contrats')"},{icon:'📦',tone:'orange',title:'Nouveau produit',sub:'Ajouter un produit',action:"show('stocks')"},{icon:'📈',tone:'turquoise',title:'Générer un rapport',sub:'Voir les rapports',action:"show('rapports')"},{icon:'⚙',tone:'blue',title:'Paramètres',sub:'Personnaliser le système',action:"show('param')"}
  ];
  ...
  return `<section id="home" class="section ${ctx.sec==='home'?'active':''}"><div class="erpDash">
    <section class="erpCard erpQuickTopCard"><div class="erpQuickTopHead"><div><h2>⚡ Actions rapides</h2><p>Accédez rapidement aux principales fonctionnalités de votre entreprise.</p></div></div><div class="erpQuickGrid">${quickActions.map(item=>`<button type="button" class="erpQuickAction" data-dashboard-search="${esc((item.title+' '+item.sub).toLowerCase())}" onclick="${item.action}"><span class="erpQuickIcon ${item.tone}">${item.icon}</span><b>${item.title}</b><small>${item.sub}</small><em>›</em></button>`).join('')}</div></section>
