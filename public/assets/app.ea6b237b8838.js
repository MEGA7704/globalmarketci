...
async function cloudLoadData(){const j=await fetchApiJsonWithRetry('/api/load',{cache:'no-store'},12000,3); CLOUD_DATA=normalizeData(j); CLOUD_LAST_ACK_DATA=cloneCloudData(CLOUD_DATA); CLOUD_DATA_READY=true; return CLOUD_DATA}
async function cloudLoadPublicData(){const j=await fetchApiJsonWithRetry('/api/public/load',{cache:'no-store'},12000,3); PUBLIC_CLIENT_SESSION=j.clientSession||null; CLOUD_DATA=normalizeData(j); CLOUD_LAST_ACK_DATA=cloneCloudData(CLOUD_DATA); CLOUD_DATA_READY=true; if(PUBLIC_CLIENT_SESSION?.clientId) window.publicShopClientId=PUBLIC_CLIENT_SESSION.clientId; else window.publicShopClientId=''; return CLOUD_DATA}
...
async function cloudStart(){
  ...
  try{
    ...
    if(sequence===CLOUD_BOOT_SEQUENCE){
      ...
      renderGlobalShop();
      setTimeout(()=>{if(!CLOUD_DATA_READY&&!CLOUD_SESSION)cloudLoadPublicData().then(()=>renderGlobalShop()).catch(()=>{})},10000);
    }
  }
}
...
function secureErrorMessage(error,fallback='Opération sécurisée impossible.'){
  ...
  if(error?.code==='STORAGE_BUSY'||error?.status===503)return 'Le service cloud est temporairement occupé. Patientez quelques secondes puis réessayez.';
  ...
  if(/service cloud n.a pas répondu|momentanément occupé|stockage.*occupé/i.test(msg)) return 'Le service cloud est temporairement occupé. Patientez quelques secondes puis réessayez.';
  return msg||fallback;
}
...
function gm56StartNotifications(){if(GM56_NOTIFY_TIMER)clearInterval(GM56_NOTIFY_TIMER);setTimeout(gm56PrimeNotifications,1800);GM56_NOTIFY_TIMER=setInterval(gm56PollNotifications,90000)}
