...
async function securePublicClientAuth(path,payload){
  let lastError=null;
  const delays=[0,450,1000,2200];
  for(let attempt=0;attempt<delays.length;attempt++){
    if(attempt)await waitMs(delays[attempt]);
    try{
      const r=await fetchWithTimeout(path,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload),cache:'no-store'},18000);
      return await readApiPayload(r);
    }catch(error){lastError=error;if(!isTransientLoginError(error)||attempt===delays.length-1)throw error;}
  }
  throw lastError||new Error('Connexion client impossible.');
}
