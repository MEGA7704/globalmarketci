...
function planActivationButtons(cid,currentCode){
  ...
  return `<div class="planActivationBox"><h3>Activation abonnement par Super Admin</h3><p>Choisissez la formule à activer. Free : 10 jours. Standard et Business : 30 jours.</p><div class="planButtons"><button class="${code==='FREE'?'active':''}" onclick="activateCompanyPlan('${cid}','FREE')">FREE — 10 jours</button><button class="${code==='STANDARD'?'active':''}" onclick="activateCompanyPlan('${cid}','STANDARD')">STANDARD — ${money(STANDARD_PLAN_AMOUNT)}/mois</button><button class="${code==='BUSINESS'?'active':''}" onclick="activateCompanyPlan('${cid}','BUSINESS')">BUSINESS — ${money(BUSINESS_PLAN_AMOUNT)}/mois</button></div></div>`;
}
...
function showFreePlanUpgradePopup(){
  ...
  box.innerHTML=`<div class="freePlanUpgradeModal">
    ...
    <div class="freePlanUpgradeStats">
      ...
      <div><small>Business</small><b>${money(BUSINESS_PLAN_AMOUNT)} / mois</b></div>
    </div>
...
PLAN BUSINESS
Le Plan Business est facturé 46 100 FCFA par mois pour une période d’activation de trente (30) jours. Il donne un accès illimité au programme complet, y compris la Marketplace et la boutique publique.
