...
/* ========================================================================
   GLOBAL MARKET V5.3 — LIVRAISON SANS FRAIS DE VILLE & MOYENS D’EXPEDITION
   ======================================================================== */
...
function gmDeliveryCityFee(config,city,subtotal,method){
  ...
  return {cityFee:0,rate:0,local:false,pickup:false};
}
function gmDeliveryConfigCityRow(city={},homeCity=''){
  ...
  return `<div class="gmDeliveryConfigRow gmDeliveryCityRow">
    ...
    <span class="gmDeliveryLocalHint">${local?'Ville de la boutique • pourcentage local automatique':'Ville desservie • aucun frais lié à la ville'}</span>
...
function gmShippingMethodRow(method={}){
  ...
  return `<div class="gmDeliveryConfigRow gmShippingMethodRow ${locked?'gmShippingMethodLocked':''}" data-method-id="${esc(locked?'retrait-boutique':String(method.id||gmMethodId(name)))}">
    ...
    <span class="gmDeliveryLocalHint">${locked?'🔒 Moyen obligatoire • frais verrouillé à 0 FCFA':'Frais appliqués selon le moyen choisi'}</span>
...
function openMarketplaceDeliveryConfig(){
  ...
  const html=`<div class="marketPayModalBackdrop" id="marketDeliveryConfigModal"><div class="marketPayModal gmDeliveryConfigModal">
    ...
    <section class="gmDeliveryConfigSection">
      <div class="gmDeliveryConfigTitle"><div><h3>2. Villes de livraison</h3><p>La ville de la boutique doit obligatoirement figurer dans la liste. Les villes servent uniquement à définir les destinations desservies : aucun frais fixe n’est associé à une ville.</p></div><button type="button" class="btn2" onclick="gmAddDeliveryCityRow()">+ Ajouter une ville</button></div>
    ...
    <section class="gmDeliveryConfigSection">
      <div class="gmDeliveryConfigTitle"><div><h3>3. Moyens d’expédition</h3><p>Chaque moyen possède son propre frais. « Retrait à la boutique » est obligatoire, limité à la ville de la boutique et verrouillé à 0 FCFA.</p></div><button type="button" class="btn2" onclick="gmAddShippingMethodRow()">+ Ajouter un moyen</button></div>
...
function gmRefreshDeliveryConfigRows(){
  ...
  document.querySelectorAll('#gmDeliveryCitiesRows .gmDeliveryCityRow').forEach(row=>{
    ...
    if(hint)hint.textContent=local?'Ville de la boutique • pourcentage local automatique':'Ville desservie • aucun frais lié à la ville';
...
function openGlobalMarketCart(){
  ...
  const shippingSelectors=[...grouped.entries()].map(([cid,rows])=>{
    const company=companyMap.get(cid)||{},pricing=gmCartShopPricing(cid,rows,company,st.city),methods=gmAvailableShippingMethods(company,st.city);
    ...
    const feeNote=pricing.pickup?'Retrait à la boutique • 0 FCFA':pricing.local?`${deliveryRateLabel(pricing.rate)} local + ${money(pricing.methodFee)} du moyen`:`Frais du moyen : ${money(pricing.methodFee)}`;
    return `<div class="gmCartShippingChoice"><label><span>${grouped.size>1?esc(company.name||'Boutique'):'Moyen d’expédition'}</span><select onchange="gmSetCartShippingMethod('${esc(cid)}',this.value)" ${methods.length?'':'disabled'}>${methodOptions||'<option>Aucun moyen disponible</option>'}</select></label><small>${esc(feeNote)}</small></div>`;
  }).join('');
  const deliveryPanel=cities.length?`<aside class="gmUnifiedDeliveryPanel"><h3>Adresse de livraison</h3><label>Ville de livraison *<select id="gmCartDeliveryCity" onchange="gmSetCartDeliveryCity(this.value)">${cityOptions}</select></label><label>Détail sur l’adresse de livraison ${requiresAddress?'*':''}<textarea id="gmCartDeliveryAddress" maxlength="500" placeholder="Quartier, rue, immeuble, porte, repère..." oninput="gmSetCartAddressDetail(this.value)">${esc(st.addressDetail||'')}</textarea></label><div class="gmCartShippingSection"><div class="gmCartShippingSectionTitle"><span>🚚</span><b>Moyen d’expédition</b></div>${shippingSelectors}</div><div class="gmUnifiedShippingAuto"><span>💰</span><div><small>Frais d’expédition automatiques</small><b>${money(feeGeneral)}</b><em>Ville de la boutique : pourcentage local + frais du moyen. Autres villes : frais du moyen uniquement.</em></div></div></aside>`:`<aside class="gmUnifiedDeliveryPanel gmDeliveryUnavailable"><h3>Adresse de livraison</h3><p>Aucune ville de livraison commune n’est configurée pour toutes les boutiques de ce panier.</p><small>Retirez un article d’une boutique ou demandez aux administrateurs d’ajouter une ville commune.</small></aside>`;
