...
function gmRefreshPublicAfterConfirmedAction(onFresh){
  scheduleCloudRefresh('public',350);
  Promise.resolve().then(()=>cloudLoadPublicData({allowStale:true,background:true})).then(()=>{if(typeof onFresh==='function')onFresh();}).catch(error=>{console.info('[V6.1.8] actualisation publique différée après opération confirmée',error?.message||error);scheduleCloudRefresh('public',1200);});
}
...
function cloudDeltaOperationCount(delta){
  let count=0;
  for(const changes of Object.values(delta?.arrays||{})) count+=(Array.isArray(changes?.upserts)?changes.upserts.length:0)+(Array.isArray(changes?.deletes)?changes.deletes.length:0);
  for(const changes of Object.values(delta?.objects||{})) count+=(Array.isArray(changes?.upserts)?changes.upserts.length:0)+(Array.isArray(changes?.deletes)?changes.deletes.length:0);
  count+=Object.keys(delta?.values||{}).length;
  return count;
}
function cloudDeltaOperationWeight(kind,key,mode,value){
  if(kind==='arrays'&&key==='orders'){
    if(mode==='deletes')return 2;
    return 3+Math.ceil((Array.isArray(value?.items)?value.items.length:0)/10);
  }
  return 1;
}
function splitCloudDelta(delta,maxWeight=32){
  const chunks=[]; let current={arrays:{},objects:{},values:{}}, weight=0;
  const flush=()=>{if(weight>0){chunks.push(current);current={arrays:{},objects:{},values:{}};weight=0;}};
  const add=(kind,key,mode,value)=>{
    const opWeight=cloudDeltaOperationWeight(kind,key,mode,value);
    if(weight>0&&weight+opWeight>maxWeight)flush();
    if(kind==='values'){current.values[key]=value;weight+=opWeight;return;}
    current[kind][key]=current[kind][key]||{upserts:[],deletes:[]};
    current[kind][key][mode].push(value);weight+=opWeight;
  };
  for(const [key,changes] of Object.entries(delta?.arrays||{})){
    for(const row of Array.isArray(changes?.upserts)?changes.upserts:[])add('arrays',key,'upserts',row);
    for(const row of Array.isArray(changes?.deletes)?changes.deletes:[])add('arrays',key,'deletes',row);
  }
  for(const [key,changes] of Object.entries(delta?.objects||{})){
    for(const row of Array.isArray(changes?.upserts)?changes.upserts:[])add('objects',key,'upserts',row);
    for(const row of Array.isArray(changes?.deletes)?changes.deletes:[])add('objects',key,'deletes',row);
  }
  for(const [key,value] of Object.entries(delta?.values||{}))add('values',key,'upserts',value);
  flush(); return chunks;
}
...
async function sendCloudSaveDelta(delta){
  const chunks=splitCloudDelta(delta,32); let total=0, storage='d1-relational-v6', last=null;
  for(let i=0;i<chunks.length;i++){
    last=await sendCloudSavePayload(JSON.stringify({delta:chunks[i],chunk:{index:i+1,total:chunks.length}}));
    total+=Number(last?.patchCount||0); storage=last?.storage||storage;
  }
  return {...(last||{}),success:true,patchCount:total,storage,chunks:chunks.length,operations:cloudDeltaOperationCount(delta)};
}
async function cloudSaveNow(d=CLOUD_DATA){
  ...
  CLOUD_SAVE_IN_FLIGHT=(async()=>{
    ...
    while(CLOUD_SAVE_PENDING&&CLOUD_SESSION){
      ...
      try{
        response=await sendCloudSaveDelta(delta);
...
function secureErrorMessage(error,fallback='Opération sécurisée impossible.'){
  ...
  if(error?.name==='AbortError'){const route=String(error?.gmUrl||'');if(route.includes('/api/login')||route.includes('/api/public/client/login'))return 'La connexion au serveur est trop lente. Votre session n’a pas été validée : réessayez sans fermer la page.';if(route.includes('/api/public/order'))return 'La confirmation de la commande prend trop de temps. Vérifiez d’abord « Mes commandes » avant de renvoyer le panier.';return 'Le serveur répond lentement. GLOBAL MARKET conserve les données déjà affichées et réessaie les actualisations non critiques automatiquement.';}
...
const GM_V614_RECENT_ORDER_PREFIX='gm-v614-recent-orders:';
function gmV614RecentOrderKey(clientId){return `${GM_V614_RECENT_ORDER_PREFIX}${String(clientId||'')}`;}
function gmV614ReadRecentOrders(clientId){try{const x=JSON.parse(localStorage.getItem(gmV614RecentOrderKey(clientId))||'[]');return Array.isArray(x)?x:[]}catch{return []}}
function gmV614MergeOrders(primary=[],fallback=[]){const m=new Map();for(const o of Array.isArray(fallback)?fallback:[])if(o?.id)m.set(String(o.id),o);for(const o of Array.isArray(primary)?primary:[])if(o?.id)m.set(String(o.id),o);return [...m.values()].sort((a,b)=>String(b.date||'').localeCompare(String(a.date||'')))}
function gmV614RememberCreatedOrders(orders){
  const client=currentGlobalClient(),cid=String(PUBLIC_CLIENT_SESSION?.clientId||client?.id||'');if(!cid||!Array.isArray(orders)||!orders.length)return;
  const normalized=orders.filter(o=>o?.id).map(o=>({...o,clientId:cid}));
  const keep=(CLOUD_DATA.orders||[]).filter(o=>String(o.clientId||'')!==cid);
  const own=(CLOUD_DATA.orders||[]).filter(o=>String(o.clientId||'')===cid);
  const merged=gmV614MergeOrders(normalized,own);
  CLOUD_DATA.orders=[...keep,...merged];
  try{localStorage.setItem(gmV614RecentOrderKey(cid),JSON.stringify(gmV614MergeOrders(normalized,gmV614ReadRecentOrders(cid)).slice(0,100)))}catch{}
}
function gmV614InjectRecentOrders(){
  const cid=String(PUBLIC_CLIENT_SESSION?.clientId||window.publicShopClientId||'');if(!cid)return;
  const local=gmV614ReadRecentOrders(cid);if(!local.length)return;
  const keep=(CLOUD_DATA.orders||[]).filter(o=>String(o.clientId||'')!==cid),own=(CLOUD_DATA.orders||[]).filter(o=>String(o.clientId||'')===cid);
  CLOUD_DATA.orders=[...keep,...gmV614MergeOrders(own,local).map(o=>({...o,clientId:cid}))];
}
...
async function finalizeGlobalMarketCart(){
  ...
  try{
    ...
    const j=await readApiPayload(r);
    gmV614RememberCreatedOrders(j.orders||[]);
    cart.length=0;refreshGlobalMarketCartBadge();
    try{await cloudLoadPublicData();}catch(e){console.info('[V6.1.4] actualisation catalogue différée après commande',e?.message||e)}
    gmV614RememberCreatedOrders(j.orders||[]);
    document.getElementById('gmGlobalCartModal')?.remove();
...
async function finalizeGlobalMarketCart(){
  ...
  try{
    ...
    const r=await fetchWithTimeout('/api/public/order',{method:'POST',headers:clientSecurityHeaders({'Content-Type':'application/json'}),body:JSON.stringify(payload),cache:'no-store'},25000);
    const j=await readApiPayload(r);gmV614RememberCreatedOrders(j.orders||[]);cart.length=0;window.GLOBAL_MARKET_CART_DELIVERY={city:'',neighborhood:'',addressDetail:'',methods:{}};refreshGlobalMarketCartBadge();try{await cloudLoadPublicData();}catch(e){console.info('[V6.1.4] actualisation catalogue différée après commande',e?.message||e)}gmV614RememberCreatedOrders(j.orders||[]);document.getElementById('gmGlobalCartModal')?.remove();
...
async function loginPublicClient(){
  ...
  try{const j=await fetchApiJsonWithRetry('/api/public/client/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({phone,password})},20000,2);PUBLIC_CLIENT_SESSION=j.session||null;window.publicShopClientId=j.client?.id||'';if(j.client){const keep=(CLOUD_DATA.marketClients||[]).filter(c=>String(c.id||'')!==String(j.client.id||''));CLOUD_DATA.marketClients=[...keep,j.client];}document.getElementById('clientLoginModal')?.remove();render();setTimeout(()=>openClientSpace(),0);gmRefreshPublicAfterConfirmedAction(()=>{if(location.hash==='#home')renderGlobalShop();});}catch(e){alert(secureErrorMessage(e,'Téléphone ou mot de passe incorrect.'));}finally{GM_CLIENT_LOGIN_BUSY=false;if(btn?.isConnected){btn.disabled=false;btn.textContent='Se connecter';}}
}
async function savePublicClientRegister(){
  ...
  try{const r=await fetchWithTimeout('/api/public/client/register',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)},20000);const j=await readApiPayload(r);PUBLIC_CLIENT_SESSION=j.session||null;window.publicShopClientId=j.client?.id||'';if(j.client){const keep=(CLOUD_DATA.marketClients||[]).filter(c=>String(c.id||'')!==String(j.client.id||''));CLOUD_DATA.marketClients=[...keep,j.client];}document.getElementById('clientRegisterModal')?.remove();render();setTimeout(()=>openClientSpace(),0);gmRefreshPublicAfterConfirmedAction(()=>{if(location.hash==='#home')renderGlobalShop();});}catch(e){alert(secureErrorMessage(e,'Inscription client impossible.'));}finally{GM_CLIENT_REGISTER_BUSY=false;if(btn?.isConnected){btn.disabled=false;btn.textContent='Créer mon compte client';}}
}
  ...
  try{
    ...
    if(tab==='orders'){
      ...
      if(Array.isArray(j.orders)&&(j.orders.length||j.authoritative)){
        ...
        const local=gmV614ReadRecentOrders(client.id);
        CLOUD_DATA.orders=[...keep,...gmV614MergeOrders(j.orders.map(o=>({...o,clientId:client.id})),local).map(o=>({...o,clientId:client.id}))];
      }
      gmV614InjectRecentOrders();
      if(j.orders?.length){window.GM_V613_ORDER_RECOVERY_RETRIES=0;}
      else if(j.recovery?.pending&&document.getElementById('clientSpaceModal')){
        const n=Number(window.GM_V613_ORDER_RECOVERY_RETRIES||0);
        if(n<4){window.GM_V613_ORDER_RECOVERY_RETRIES=n+1;setTimeout(()=>{if(document.getElementById('clientSpaceModal'))gmV611RefreshClientSpace('orders').then(()=>{if(document.getElementById('clientSpaceModal'))gm56RenderClientSpaceTab('orders')})},2500+1500*n);}
      }
      return j;
    }
...
async function gmV612ForceClientOrderRecovery(){
  window.GM_V613_ORDER_RECOVERY_RETRIES=0;
