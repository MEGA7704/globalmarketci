...
renderGlobalShop=function(){
  ...
  const header=document.querySelector('.gmUniversalHeader');if(header&&!document.querySelector('.gm56AboutLaunch'))header.insertAdjacentHTML('afterend',`<section class="gm56AboutLaunch"><div><span>GLOBAL MARKET</span><b>Gestion complète + boutique en ligne pour entreprises, vendeurs et clients.</b></div><button onclick="openGlobalAboutPage()">À propos</button></section>`);
...
function openGlobalAboutPage(){
  app.innerHTML=`<main class="gm56AboutPage">${globalUniversalHeader('about')}<section class="gm56AboutIntro"><span>À PROPOS DE GLOBAL MARKET</span><h1>Une plateforme pour gérer votre activité et vendre en ligne</h1><p>GLOBAL MARKET réunit dans un même environnement la gestion commerciale d’une entreprise et une marketplace publique où les clients peuvent découvrir, commander et suivre des produits et services.</p><button onclick="goGlobalMarketHome()">Retour à l’accueil</button></section><section class="gm56AboutGrid"><article><div>🏪</div><h2>Présentation de GLOBAL MARKET</h2><p>Chaque entreprise dispose d’un espace de gestion professionnel, d’une boutique publique, d’un catalogue, du suivi des commandes, des clients, des stocks, des ventes, des rapports et des paramètres Marketplace.</p></article><article><div>🎯</div><h2>Notre mission</h2><p>Faciliter la digitalisation du commerce local, rapprocher vendeurs et clients et offrir un outil simple pour gérer les opérations quotidiennes avec plus de visibilité, de suivi et de confiance.</p></article><article><div>👥</div><h2>Qui peut utiliser GLOBAL MARKET ?</h2><p>Commerçants, entreprises, prestataires de services, boutiques, indépendants, administrateurs, caissiers et clients souhaitant acheter auprès de plusieurs vendeurs depuis un même compte.</p></article></section><section class="gm56AboutGuide"><div class="gm56AboutGuideHead"><span>GUIDE D’UTILISATION</span><h2>Comment créer votre boutique ?</h2></div><div class="gm56GuideSteps"><article><b>1</b><h3>Créer ma boutique</h3><p>Depuis l’accueil, cliquez sur « Créer ma boutique ou se connecter » puis choisissez la création d’entreprise.</p></article><article><b>2</b><h3>Renseigner l’entreprise</h3><p>Complétez la fiche avec l’identité de l’entreprise, le responsable, les contacts et vos identifiants sécurisés.</p></article><article><b>3</b><h3>Ajouter vos offres</h3><p>Enregistrez vos produits et services dans le stock général puis rendez-les visibles dans la Marketplace.</p></article><article><b>4</b><h3>Configurer la Marketplace</h3><p>Ajoutez vos moyens de paiement, villes, quartiers, expéditions et frais applicables à vos clients.</p></article><article><b>5</b><h3>Partager et vendre</h3><p>Ouvrez votre boutique publique, partagez son lien et recevez commandes et messages directement dans votre espace.</p></article></div></section><section class="gm56AboutFinal"><div>${globalMarketBrandMark()}</div><h2>GLOBAL MARKET</h2><p>Produits & services en toute confiance.</p><button onclick="openGlobalBusinessAuth('register')">Créer ma boutique</button></section></main>`;initFlexibleHorizontalMenu();
}
...
let GM56_NOTIFY_CONTEXT='',GM56_NOTIFY_ORDERS=new Map(),GM56_NOTIFY_MESSAGES=new Set(),GM56_NOTIFY_TIMER=null;
...
async function gm56PollNotifications(){
  const ctx=gm56NotificationContext();if(ctx==='guest'||document.hidden||CLOUD_SAVE_PENDING)return;if(ctx!==GM56_NOTIFY_CONTEXT){gm56PrimeNotifications();return}
  ...
  }catch(e){console.warn('Notifications GLOBAL MARKET non actualisées',e)}
}
function gm56StartNotifications(){if(GM56_NOTIFY_TIMER)clearInterval(GM56_NOTIFY_TIMER);setTimeout(gm56PrimeNotifications,1200);GM56_NOTIFY_TIMER=setInterval(gm56PollNotifications,12000)}
