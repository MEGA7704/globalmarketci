...
async function clientCancelOrder(orderId,companyId){
  ...
  try{await securePublicPost('/api/public/order/action',{orderId,action:'cancel'});await cloudLoadPublicData();document.getElementById('clientOrderDetailsPremium')?.remove();document.getElementById('clientTrackingModal')?.remove();openClientSpace(companyId);}catch(e){alert(secureErrorMessage(e,'Annulation impossible.'))}
}
async function clientConfirmReceipt(orderId,companyId){
  ...
  try{await securePublicPost('/api/public/order/action',{orderId,action:'confirm_receipt'});await cloudLoadPublicData();document.getElementById('clientOrderDetailsPremium')?.remove();document.getElementById('clientTrackingModal')?.remove();openClientSpace(companyId);}catch(e){alert(secureErrorMessage(e,'Confirmation impossible.'))}
}
...
async function secureEmployeeLogin(payload){
  ...
  for(let attempt=0;attempt<retryDelays.length;attempt++){
    if(attempt>0){
      setLoginMessage(`Service cloud occupé : nouvelle tentative ${attempt+1}/${retryDelays.length}…`,'info');
      await waitMs(retryDelays[attempt]);
    }
    try{
      const r=await fetchWithTimeout('/api/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload),cache:'no-store'},18000);
      return await readApiPayload(r);
    }catch(error){
      lastError=error;
      if(!isTransientLoginError(error)||attempt===retryDelays.length-1)throw error;
    }
  }
  throw lastError||new Error('Connexion cloud impossible.');
}
function secureErrorMessage(error,fallback='Opération sécurisée impossible.'){
  ...
  if(isTransientLoginError(error)) return 'Le service cloud ne répond pas encore après plusieurs tentatives. Vérifiez votre connexion Internet puis réessayez.';
