...
function gmCartShopPricing(companyId,rows,company,city){
  ...
  const minimumOutside=10000,minimumMet=subtotal>=minimumOutside;
  ...
  if(!selected)return {subtotal,cityFee:0,methodFee:0,deliveryFee:0,total:subtotal,method:null,neighborhood:null,rate:0,valid:false,local:false,pickup:false,minimumOutside,minimumMet};
  const methodFee=Math.max(0,Math.round(Number(selected.fee||0)));
  return {subtotal,cityFee:0,methodFee,deliveryFee:methodFee,total:subtotal+methodFee,method:selected,neighborhood:null,rate:0,valid:minimumMet,local:false,pickup:false,minimumOutside,minimumMet};
}
function globalCartPricing(){
...
function openGlobalMarketCart(){
  ...
  const outsideMinimumFailures=pricings.filter(x=>!x.pricing.local&&!x.pricing.minimumMet);
  ...
  const minWarning=outsideMinimumFailures.length?`<div class="gmOutsideMinimumWarning"><b>Minimum hors ville : 10 000 FCFA par boutique.</b>${outsideMinimumFailures.map(x=>`<span>${esc(x.company.name||'Boutique')} : ${money(x.pricing.subtotal)} — ajoutez ${money(Math.max(0,10000-x.pricing.subtotal))}</span>`).join('')}</div>`:'';
  ...
  const orderDisabled=!allMethodsValid||!cities.length||(hasLocal&&!localNeighborhoods.length)||outsideMinimumFailures.length>0;
...
async function finalizeGlobalMarketCart(){
  ...
  const outsideBelowMinimum=shopPricings.filter(p=>!p.local&&Number(p.subtotal||0)<10000);
  if(outsideBelowMinimum.length)return alert('Toute commande hors de la ville de la boutique doit atteindre au moins 10 000 FCFA par boutique.');
