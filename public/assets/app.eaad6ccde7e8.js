...
async function fetchApiJsonWithRetry(url,opts={},timeoutMs=12000,attempts=4){
  let lastError=null;
  for(let attempt=0;attempt<attempts;attempt++){
    try{const r=await fetchWithTimeout(url,opts,timeoutMs);return await readApiPayload(r)}catch(error){
      ...
      const backoff=[500,1200,2600,5200,9000][Math.min(attempt,4)];
      await waitMs(Math.max(serverDelay,backoff)+Math.floor(Math.random()*250));
    }
  }
...
async function cloudLoadData(){const j=await fetchApiJsonWithRetry('/api/load',{cache:'no-store'},12000,4); CLOUD_DATA=normalizeData(j); CLOUD_LAST_ACK_DATA=cloneCloudData(CLOUD_DATA); CLOUD_DATA_READY=true; return CLOUD_DATA}
async function cloudLoadPublicData(){const j=await fetchApiJsonWithRetry('/api/public/load',{cache:'no-store'},12000,4); PUBLIC_CLIENT_SESSION=j.clientSession||null; CLOUD_DATA=normalizeData(j); CLOUD_LAST_ACK_DATA=cloneCloudData(CLOUD_DATA); CLOUD_DATA_READY=true; if(PUBLIC_CLIENT_SESSION?.clientId) window.publicShopClientId=PUBLIC_CLIENT_SESSION.clientId; else window.publicShopClientId=''; return CLOUD_DATA}
...
function renderGlobalMarketPagination(pages){
  ...
  if(pages<=1){nav.innerHTML='<button disabled aria-label="Page précédente">‹</button><button class="active" aria-current="page">1</button><button disabled aria-label="Page suivante">›</button>';return}
...
function secureErrorMessage(error,fallback='Opération sécurisée impossible.'){
  if(error?.status===429){if(error?.code==='RATE_LIMITED')return error.message||'Trop de tentatives. Réessayez plus tard.';return 'Le service cloud est très sollicité. Plusieurs nouvelles tentatives ont déjà été effectuées automatiquement. Réessayez dans quelques instants.';}
  ...
  if(/service cloud n.a pas répondu|momentanément occupé|stockage est momentanément occupé/i.test(msg)) return 'Le service cloud est très sollicité. Plusieurs nouvelles tentatives ont déjà été effectuées automatiquement. Réessayez dans quelques instants.';
  return msg||fallback;
}
...
function gm56StartNotifications(){if(GM56_NOTIFY_TIMER)clearInterval(GM56_NOTIFY_TIMER);setTimeout(gm56PrimeNotifications,1800);GM56_NOTIFY_TIMER=setInterval(gm56PollNotifications,45000)}
