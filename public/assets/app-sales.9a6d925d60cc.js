...
function publicShopApplyFilters(){
  ...
  document.querySelectorAll('.psProductCard').forEach(card=>{
    ...
    const show=matchesQuery&&matchesCategory&&matchesMode&&matchesPopular;
    card.hidden=!show;
    if(show) visible++;
  });
  const count=document.getElementById('publicShopResultCount');
  if(count) count.textContent=visible+' offre'+(visible>1?'s':'')+' disponible'+(visible>1?'s':'');
...
function renderPublicShop(slug){
  ...
  window.publicShopCatalogMode='all'; window.publicShopPopularGroup=''; publicShopApplyFilters();
}
