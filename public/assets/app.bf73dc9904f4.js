...
function gmClientOrderCard(o,shop){
  ...
  const deleteArea=!canDelete?'<span class="gmOrderLocked">Paiement confirmé</span>':(clientCancelled||gmOrderCancelled(o)?`<button type="button" class="gmOrderDeleteBtn" onclick="deleteMarketplaceOrder('${esc(o.id)}',false)">Supprimer</button>`:`<button type="button" class="gmOrderCancelBtn" onclick="cancelClientMarketplaceOrder('${esc(o.id)}')">Annuler</button>`);
