...
async function cloudLoadData(options={}){
  ...
  try{
    const j=await fetchApiJsonWithRetry('/api/load',{cache:'no-store'},15000,6);
...
async function cloudLoadPublicData(options={}){
  const allowStale=options.allowStale!==false;
  try{
    const j=await fetchApiJsonWithRetry('/api/public/load',{cache:'no-store'},15000,6);
    PUBLIC_CLIENT_SESSION=j.clientSession||PUBLIC_CLIENT_SESSION||null; CLOUD_DATA=normalizeData(j); CLOUD_LAST_ACK_DATA=cloneCloudData(CLOUD_DATA); CLOUD_DATA_READY=true; if(PUBLIC_CLIENT_SESSION?.clientId) window.publicShopClientId=PUBLIC_CLIENT_SESSION.clientId; else window.publicShopClientId=''; return CLOUD_DATA;
  }catch(error){
...
function gm56StartNotifications(){if(GM56_NOTIFY_TIMER)clearInterval(GM56_NOTIFY_TIMER);setTimeout(gm56PrimeNotifications,1800);GM56_NOTIFY_TIMER=setInterval(gm56PollNotifications,120000)}
