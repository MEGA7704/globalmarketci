...
function reportSaleRowInteractiveTarget(target){
  return Boolean(target?.closest?.('button,input,select,textarea,a,label'));
}
function openReportSaleRow(event,sid){
  if(reportSaleRowInteractiveTarget(event?.target)) return;
  openEditSalePopup(sid);
}
function openReportSaleRowFromKey(event,sid){
  if(!event || (event.key!=='Enter' && event.key!==' ')) return;
  if(reportSaleRowInteractiveTarget(event.target)) return;
  event.preventDefault();
  openEditSalePopup(sid);
...
const SERVICE_REPORT_PAGE_SIZE=30;
function changeServiceReportPage(delta){
  const next=Math.max(1,Number(window.serviceReportPage||1)+Number(delta||0));
  window.serviceReportPage=next;
  applyServiceReportPeriodFilter(false);
  document.querySelector('#serviceReportSalesTable')?.scrollIntoView({behavior:'smooth',block:'start'});
}
function applyServiceReportPeriodFilter(resetPage=true){
  const table=$('#serviceReportSalesTable'); if(!table) return;
  toggleServiceReportPeriodFields(false);
  if(resetPage!==false) window.serviceReportPage=1;
  ...
  document.querySelectorAll('#serviceReportSalesTable tbody tr.serviceSaleRow').forEach(r=>{
    ...
    if(ok){
      ...
      profit+=Number(r.dataset.profit||0);
      qty+=Number(r.dataset.qty||0);
    }
  });
  const totalPages=Math.max(1,Math.ceil(count/SERVICE_REPORT_PAGE_SIZE));
  const page=Math.min(totalPages,Math.max(1,Number(window.serviceReportPage||1)));
  window.serviceReportPage=page;
  const first=(page-1)*SERVICE_REPORT_PAGE_SIZE;
  const last=Math.min(first+SERVICE_REPORT_PAGE_SIZE,count);
  matched.forEach((row,index)=>{row.style.display=(index>=first&&index<last)?'':'none';});
  ...
  const pageLabel=$('#serviceReportPageLabel'); if(pageLabel) pageLabel.textContent=`Page ${page} sur ${totalPages}`;
  const pageRange=$('#serviceReportPageRange'); if(pageRange) pageRange.textContent=count?`${first+1} à ${last} sur ${count} ligne(s)`:'0 ligne';
  const previous=$('#serviceReportPrevious'); if(previous) previous.disabled=page<=1;
  const next=$('#serviceReportNext'); if(next) next.disabled=page>=totalPages;
  const pagination=$('#serviceReportPagination'); if(pagination) pagination.classList.toggle('is-empty',count===0);
...
function serviceReport(items,sales,admin=false){
  ...
  <div class="serviceReportPagination no-print" id="serviceReportPagination"><button type="button" id="serviceReportPrevious" onclick="changeServiceReportPage(-1)">← Précédent</button><span><b id="serviceReportPageLabel">Page 1 sur 1</b><small id="serviceReportPageRange">0 ligne</small></span><button type="button" id="serviceReportNext" class="next" onclick="changeServiceReportPage(1)">Suivant →</button></div>
...
function clientContractSection(clients,admin,sales){
  return `<div class="contractTopActions no-print">
    ...
    <button class="contractListBtn" onclick="showContractClientList(this)">Liste des clients sous contrat</button>
  </div>
  ${clientContractForm()}
  <div id="contractClientListPage" class="contractClientListPage hidden" aria-hidden="true">${clientContractList(clients,admin)}</div>
...
function showContractClientList(button){
  const page=document.querySelector('#contractClientListPage'); if(!page) return;
  const opening=page.classList.contains('hidden');
  page.classList.toggle('hidden',!opening);
  page.setAttribute('aria-hidden',opening?'false':'true');
  if(button) button.textContent=opening?'Masquer la liste des clients':'Liste des clients sous contrat';
  if(opening) setTimeout(()=>page.scrollIntoView({behavior:'smooth',block:'start'}),40);
}
