...
function saleProfessionalCart(cartSales,clients=[]){
  ...
  const lineHtml=rows.map((s,index)=>{
    ...
    return `<article class="saleProCartLine">
      <div class="saleProCartLineHead"><span class="saleProCartLineIcon ${product?'product':'service'}">${product?'📦':'🧾'}</span><div><b>${esc(s.name||'Article')}</b><small>${esc(s.itemCode||s.category||'')} • ${product?'Produit':'Service'}</small></div><button type="button" class="saleProCartRemove" aria-label="Retirer du panier" onclick="removePendingCartLine('${esc(s.id)}')">×</button></div>
      <div class="saleProCartLineBody">
        <div class="saleProQty"><button type="button" onclick="adjustPendingCartLine('${esc(s.id)}',-1)">−</button><input type="number" min="1" value="${Number(s.qty||1)}" onchange="setPendingCartLineQty('${esc(s.id)}',this.value)"><button type="button" onclick="adjustPendingCartLine('${esc(s.id)}',1)">+</button></div>
