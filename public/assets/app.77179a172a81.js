...
async function submitClientOrderPayment(orderId,method){
  ...
  try{await securePublicPost('/api/public/order/action',{orderId,action:'declare_payment',paymentMethod:method,transactionId});await gmRefreshPublicDataAfterAction();document.getElementById('gmClientOrderPaymentModal')?.remove();document.getElementById('clientSpaceModal')?.remove();openClientSpace();alert('Paiement transmis à la boutique. En attente de confirmation par l’administrateur.');}catch(e){alert(secureErrorMessage(e,'Transmission du paiement impossible.'))}
}
