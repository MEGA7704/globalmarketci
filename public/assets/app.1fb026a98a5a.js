...
function gmRefreshPublicAfterConfirmedAction(onFresh){
  ...
  Promise.resolve().then(()=>cloudLoadPublicData({allowStale:true,background:true})).then(()=>{if(typeof onFresh==='function')onFresh();}).catch(error=>{console.info('[V6.1.7] actualisation publique différée après opération confirmée',error?.message||error);scheduleCloudRefresh('public',1200);});
}
