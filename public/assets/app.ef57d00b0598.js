...
/* ========================================================================
   GLOBAL MARKET V5.4 — QUARTIERS LOCAUX & EXPEDITION HORS VILLE
   ======================================================================== */
...
function gmCartShopPricing(companyId,rows,company,city){
  ...
  if(local){
    ...
    if(!neighborhood)return {subtotal,cityFee:0,methodFee:0,deliveryFee:0,total:subtotal,method:null,neighborhood:null,rate:0,valid:false,local:true,pickup:false};
    ...
    return {subtotal,cityFee,methodFee:0,deliveryFee:cityFee,total:subtotal+cityFee,method:pickupMethod,neighborhood,rate,valid:true,local:true,pickup};
  }
...
function openGlobalMarketCart(){
  ...
  const neighborhoodSelector=hasLocal?`<div class="gmCartNeighborhoodSection"><div class="gmCartShippingSectionTitle"><span>📍</span><b>Quartier de livraison</b></div><label>Quartier de livraison *<select id="gmCartDeliveryNeighborhood" onchange="gmSetCartNeighborhood(this.value)">${neighborhoodOptions||'<option>Aucun quartier commun configuré</option>'}</select></label><small>${gmDeliveryToken(st.neighborhood)==='RETRAIT A LA BOUTIQUE'?'Retrait à la boutique : aucun frais local.':'Le frais de base local est calculé automatiquement par pourcentage sur le sous-total.'}</small></div>`:'';
  const shippingSelectors=pricings.filter(x=>!x.pricing.local).map(({cid,company,pricing})=>{
    ...
    return `<div class="gmCartShippingChoice"><label><span>${grouped.size>1?esc(company.name||'Boutique'):'Moyen d’expédition'}</span><select onchange="gmSetCartShippingMethod('${esc(cid)}',this.value)" ${methods.length?'':'disabled'}>${methodOptions||'<option>Aucun moyen disponible</option>'}</select></label><small>Hors ville de la boutique : seuls les frais du moyen choisi sont appliqués.</small></div>`;
  }).join('');
  const shippingSection=hasOutside?`<div class="gmCartShippingSection"><div class="gmCartShippingSectionTitle"><span>🚚</span><b>Moyen d’expédition</b></div>${shippingSelectors}</div>`:'';
  const summary=`<div class="gmUnifiedCartSummary gmDeliveryCartSummary"><div><span>Sous-total produits & services</span><b>${money(subtotalGeneral)}</b></div><div><span>Frais d’expédition</span><b>${money(feeGeneral)}</b></div><div class="grand"><span>Total général</span><b>${money(total)}</b></div></div>`;
  const deliveryPanel=cities.length?`<aside class="gmUnifiedDeliveryPanel"><h3>Adresse de livraison</h3><label>Ville de livraison *<select id="gmCartDeliveryCity" onchange="gmSetCartDeliveryCity(this.value)">${cityOptions}</select></label><label>Détail sur l’adresse de livraison ${requiresAddress?'*':''}<textarea id="gmCartDeliveryAddress" maxlength="500" placeholder="Quartier, rue, immeuble, porte, repère..." oninput="gmSetCartAddressDetail(this.value)">${esc(st.addressDetail||'')}</textarea></label>${neighborhoodSelector}${shippingSection}${summary}</aside>`:`<aside class="gmUnifiedDeliveryPanel gmDeliveryUnavailable"><h3>Adresse de livraison</h3><p>Aucune ville de livraison commune n’est configurée pour toutes les boutiques de ce panier.</p><small>Retirez un article d’une boutique ou demandez aux administrateurs d’ajouter une ville commune.</small></aside>`;
  const orderDisabled=!allMethodsValid||!cities.length||(hasLocal&&!localNeighborhoods.length);
  document.body.insertAdjacentHTML('beforeend',`<div class="gmHomeAuthBackdrop" id="gmGlobalCartModal" onclick="if(event.target.id==='gmGlobalCartModal')this.remove()"><section class="gmHomeAuthCard gmUnifiedCartCard"><button class="gmHomeAuthClose" onclick="document.getElementById('gmGlobalCartModal')?.remove()">×</button><span class="gmHomeModalKicker">PANIER UNIQUE CLIENT</span><div class="gmUnifiedCartTitle"><div><h2>Mon panier GLOBAL MARKET</h2><p>Un seul panier pour vos produits et services de plusieurs boutiques.</p></div><span class="gmCartClientChip">${officialShopIcon('user')} ${esc(client.name||'Client connecté')}</span></div><div class="gmUnifiedCartLayout"><div class="gmUnifiedCartMain"><div class="gmUnifiedCartGroups">${groupsHtml}</div></div>${deliveryPanel}</div><div class="gmUnifiedCartActions"><button class="btn2" onclick="clearGlobalClientCart();document.getElementById('gmGlobalCartModal')?.remove()">Vider le panier</button><button class="gmHomePrimaryAuth" onclick="finalizeGlobalMarketCart()" ${orderDisabled?'disabled':''}>Passer ma commande</button></div><p class="gmUnifiedCartPayment">Paiement appliqué : <b>Paiement à la livraison</b>.</p></section></div>`);
}
async function finalizeGlobalMarketCart(){
