...
function openClientSpace(){
  ...
  document.body.insertAdjacentHTML('beforeend',`<div class="marketPayModalBackdrop" id="clientSpaceModal"><div class="marketPayModal clientSpaceBox gmGlobalClientSpace gmClientOrdersWorkspace"><button class="marketPayClose" onclick="document.getElementById('clientSpaceModal')?.remove()">×</button><div class="gmClientSpaceHead"><div><span class="gmHomeModalKicker">ESPACE CLIENT GLOBAL MARKET</span><h2>${esc(client.name)}</h2><p>${esc(client.phone)} ${client.email?'— '+esc(client.email):''}</p></div><div class="gmClientSpaceHeadActions"><button type="button" class="gmClientAccountBtn" onclick="openClientAccountInSpace()">${officialShopIcon('user')} Mon compte</button></div></div><div class="gmClientOrdersTitle"><div><h3>Mes commandes par boutique</h3><p>Chaque lot est traité par la boutique concernée. Le paiement devient disponible après validation du vendeur.</p></div><span>${orders.length} commande(s)</span></div><div class="gmClientOrdersScroll">${groupsHtml}</div><div class="marketPayActions"><button onclick="document.getElementById('clientSpaceModal')?.remove()">Fermer</button></div></div></div>`);
}
...
async function saveGlobalClientAccount(){
  ...
  try{
    ...
    await gmRefreshPublicDataAfterAction();document.getElementById('clientSpaceModal')?.remove();render();setTimeout(()=>openClientAccountInSpace(),0);alert('Votre compte client a été mis à jour.');
...
async function deleteClientMessages(ids=[],all=false){try{const r=await fetchWithTimeout('/api/public/message/delete',{method:'POST',headers:clientSecurityHeaders({'Content-Type':'application/json'}),body:JSON.stringify({ids,all}),cache:'no-store'},12000);await readApiPayload(r);await gmRefreshPublicDataAfterAction();document.getElementById('gmClientMessagesModal')?.remove();openClientMessagesPopup()}catch(e){alert(secureErrorMessage(e,'Suppression des messages impossible.'))}}
...
async function loginPublicClient(){
  ...
  try{const r=await fetchWithTimeout('/api/public/client/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({phone,password})},12000);const j=await readApiPayload(r);PUBLIC_CLIENT_SESSION=j.session||null;window.publicShopClientId=j.client?.id||'';await cloudLoadPublicData();document.getElementById('clientLoginModal')?.remove();render();setTimeout(()=>{gmPollRealtimeNotifications();openClientSpace();},0);}catch(e){alert(secureErrorMessage(e,'Téléphone ou mot de passe incorrect.'));}finally{GM_CLIENT_LOGIN_BUSY=false;if(btn?.isConnected){btn.disabled=false;btn.textContent='Se connecter';}}
}
async function savePublicClientRegister(){
  ...
  try{const r=await fetchWithTimeout('/api/public/client/register',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)},12000);const j=await readApiPayload(r);PUBLIC_CLIENT_SESSION=j.session||null;window.publicShopClientId=j.client?.id||'';await cloudLoadPublicData();document.getElementById('clientRegisterModal')?.remove();render();setTimeout(()=>{gmPollRealtimeNotifications();openClientSpace();},0);}catch(e){alert(secureErrorMessage(e,'Inscription client impossible.'));}finally{GM_CLIENT_REGISTER_BUSY=false;if(btn?.isConnected){btn.disabled=false;btn.textContent='Créer mon compte client';}}
}
