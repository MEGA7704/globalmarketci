...
function showClientOrderPaymentMethod(orderId,method){
  ...
  const target=document.getElementById('gmOrderPaymentChoice');if(target)target.innerHTML=paymentBox+`<div class="gmOrderTransactionForm"><label>ID de la transaction<input id="gmOrderTransactionId" placeholder="Saisissez l’identifiant de la transaction"></label><button type="button" onclick="submitClientOrderPayment('${esc(o.id)}','${esc(method)}')">J’ai payé</button><small class="gmPaymentWhatsappHint">Après validation, WhatsApp de la boutique s’ouvrira avec les informations de cette commande déjà préparées.</small></div>`;
}
...
async function submitClientOrderPayment(orderId,method){
  ...
  if(!order)return alert('Commande introuvable.');
  const company=(d.companies||[]).find(c=>String(c.id)===String(order.companyId))||{};
  const whatsappNumbers=marketplaceWhatsappNumbers(company);
  const primaryWhatsapp=whatsappNumbers[0]||'';
  let whatsappWindow=null;
  if(primaryWhatsapp){try{whatsappWindow=window.open('about:blank','_blank');if(whatsappWindow)whatsappWindow.opener=null;}catch{whatsappWindow=null;}}
  const btn=typeof event!=='undefined'?event.currentTarget:null;if(btn){btn.disabled=true;btn.textContent='Transmission…';}
  try{
    await securePublicPost('/api/public/order/action',{orderId,action:'declare_payment',paymentMethod:method,transactionId});
    if(primaryWhatsapp){
      const message=clientOrderPaymentWhatsappMessage(order,company,method,transactionId);
      const clean=normalizeMarketplaceWhatsappNumber(primaryWhatsapp);
      const whatsappUrl=`https://wa.me/${encodeURIComponent(clean)}?text=${encodeURIComponent(message)}`;
      if(whatsappWindow&&!whatsappWindow.closed)whatsappWindow.location.replace(whatsappUrl);else window.location.href=whatsappUrl;
    }
    await gmRefreshPublicDataAfterAction();
    document.getElementById('gmClientOrderPaymentModal')?.remove();document.getElementById('clientSpaceModal')?.remove();openClientSpace();
    if(primaryWhatsapp)alert('Paiement déclaré. WhatsApp de la boutique a été ouvert avec les informations de la commande. Vérifiez le message puis appuyez sur Envoyer dans WhatsApp.');
    else alert('Paiement déclaré, mais aucun numéro WhatsApp de commande n’est configuré pour cette boutique. La déclaration reste visible dans GLOBAL MARKET CI.');
  }catch(e){
