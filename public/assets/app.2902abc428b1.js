...
async function gmV611RefreshClientSpace(tab='orders'){
  ...
  try{
    ...
    if(tab==='orders'){
      const j=await fetchApiJsonWithRetry('/api/v6/client/orders?page=1&pageSize=100',{cache:'no-store'},12000,2);
      if(Array.isArray(j.orders)){
        const keep=(CLOUD_DATA.orders||[]).filter(o=>String(o.clientId||'')!==String(client.id));
        CLOUD_DATA.orders=[...keep,...j.orders];
      }
      return j;
    }
  }catch(e){console.info('[V6.1.1] historique client en cours de restauration',e?.message||e)}
  return null;
}
