...
function renderGlobalShop(){
  ...
  app.innerHTML=`<main class="gmHomePage">
    <header class="gmHomeHeader">
      <button type="button" class="gmHomeBrand" onclick="location.hash='#home';renderGlobalShop()">${globalMarketBrandMark()}<span><b>GLOBAL MARKET</b><small>Produits & services en toute confiance</small></span></button>
      <form class="gmHomeHeaderSearch" onsubmit="event.preventDefault();syncGlobalMarketSearch('gmHeaderSearch')"><input id="gmHeaderSearch" type="search" placeholder="Rechercher un produit, un service ou une entreprise..." aria-label="Recherche globale"><button type="submit" aria-label="Rechercher">${officialShopIcon('search')}</button></form>
      <div class="gmHomeHeaderActions"><button type="button" class="gmHomeBusinessBtn" onclick="openGlobalBusinessAuth()">${officialShopIcon('bag')}<span><b>Créer ma boutique</b><small>ou se connecter</small></span></button><button type="button" class="gmHomeClientBtn" onclick="openGlobalClientAuth('login')">${officialShopIcon('user')}<span><b>Créer un compte client</b><small>ou se connecter</small></span></button><button type="button" class="gmHomeHeaderCart" onclick="openGlobalMarketCart()" aria-label="Ouvrir le panier">${officialShopIcon('cart')}<i id="gmGlobalCartBadge">${globalMarketCartCount()}</i></button></div>
    </header>
  ...
  requestAnimationFrame(()=>{filterGlobalShop(false);refreshGlobalMarketCartBadge()});
}
...
function renderPublicShop(slug){
  ...
  app.innerHTML=`<div class="publicShop officialStore">
    <header class="officialHeader">
      ...
      <nav class="officialNav officialNavSimplified">
        <div class="officialNavRight"><button type="button" onclick="openCompanyRegistrationFromShop()">Créer ma Boutique</button></div>
      </nav>
    </header>
  ...
  requestAnimationFrame(()=>filterOfficialShop());
}
