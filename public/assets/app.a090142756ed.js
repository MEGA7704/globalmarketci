...
function handleCloudSaveFailure(e){
  console.error(e);
  if(e?.status===401){CLOUD_SESSION=null; alert('Votre session a expiré. Reconnectez-vous.'); renderLogin(); return;}
...
async function cloudStart(){
  ...
  const publicRoute=!location.hash||location.hash==='#home'||location.hash.startsWith('#boutique-global')||location.hash.startsWith('#boutique/');
  CLOUD_SESSION=null;
  ...
  if(publicRoute){
    app.innerHTML='<main class="gmHomeLoader"><div class="gmHomeLoaderMark">GM</div><h1>GLOBAL MARKET</h1><p>Chargement de votre marketplace produits & services…</p></main>';
  }else{
    renderLogin();
  }
  ...
  try{
    ...
    if(sequence===CLOUD_BOOT_SEQUENCE){
      ...
      CLOUD_DATA=defaultData();
      CLOUD_DATA_READY=false;
      if(publicRoute) renderGlobalShop(); else renderLogin();
    }
  }
}
...
function openCompanyRegistrationFromShop(){
  location.hash='#connexion';
  renderLogin();
  setTimeout(()=>openRegisterPopup(),0);
}
...
function renderExpired(c,st){app.innerHTML=globalUniversalHeader('public',{restricted:true})+`<div class="wrap"><div class="card" style="max-width:720px;margin:80px auto;text-align:center"><div class="brand">GLOBAL MARKET</div><h1>Abonnement ${esc(st)}</h1><p class="sub">L’accès de l’entreprise <b>${esc(c?.name)}</b> est actuellement ${esc(st)}. Contactez MEGA SERVICES DIABO pour renouveler ou réactiver l’abonnement.</p><p><b>+225 0777041790</b><br>megaservicediabo@gmail.com</p><button onclick="logout()">Retour connexion</button></div></div>`}
function render(){const hash=location.hash||'#home'; if(hash.startsWith('#boutique/')) return renderPublicShop(hash.split('/')[1]||''); if(hash==='#connexion') return renderLogin(); if(hash==='#home'||hash==='#'||hash.startsWith('#boutique-global')) return renderGlobalShop(); const {user,company}=current(); if(!user) return renderGlobalShop(); if(user.role==='superadmin') return renderSuper(); const st=statusCompany(company); if(['expired','blocked','suspended'].includes(st)) return renderExpired(company,st); renderDash('home')}
...
function globalMarketDemoItems(){return [
  {id:'demo-electronics',demo:true,name:'Pack Électronique Premium',cat:'Électronique',desc:'Ensemble complet d’accessoires électroniques de qualité pour un quotidien connecté.',price:120000,shop:'GLOBAL MARKET Sélection',photo:'assets/demo-electronics.jpg',type:'product'},
  {id:'demo-fashion',demo:true,name:'Collection Mode Homme',cat:'Mode & Vêtements',desc:'Vêtements tendance et élégants pour hommes, avec finitions soignées.',price:25000,shop:'GLOBAL MARKET Sélection',photo:'assets/demo-fashion.jpg',type:'product'},
  {id:'demo-sofa',demo:true,name:'Canapé 3 Places Premium',cat:'Maison & Décoration',desc:'Canapé confortable au design moderne pour sublimer votre intérieur.',price:250000,shop:'GLOBAL MARKET Sélection',photo:'assets/demo-sofa.jpg',type:'product'},
  {id:'demo-beauty',demo:true,name:'Coffret Soin Visage',cat:'Beauté & Santé',desc:'Coffret de soins complet pour une routine beauté simple et élégante.',price:18000,shop:'GLOBAL MARKET Sélection',photo:'assets/demo-beauty.jpg',type:'product'},
  {id:'demo-sport',demo:true,name:'Chaussures de Sport Pro',cat:'Sport & Loisirs',desc:'Chaussures performantes conçues pour le confort et les activités quotidiennes.',price:35000,shop:'GLOBAL MARKET Sélection',photo:'assets/demo-sport.jpg',type:'product'},
  {id:'demo-bag',demo:true,name:'Sac à Main Élégance',cat:'Mode & Accessoires',desc:'Sac élégant et pratique, pensé pour accompagner toutes vos occasions.',price:45000,shop:'GLOBAL MARKET Sélection',photo:'assets/demo-bag.jpg',type:'product'}
]}
function globalMarketCardHtml(i,company={}){
  ...
  const type=demo?(i.type||'product'):(isBoutiqueItem(i)?'product':'service');
  const price=demo?Number(i.price||0):itemMarketPrice(i);
  const dt=demo?Date.now()-Math.random()*86400000*8:new Date(i.createdAt||i.date||i.updatedAt||0).getTime()||0;
  ...
  const actions=demo?`<button type="button" class="gmHomeGhostBtn" disabled>Voir la boutique</button><button type="button" class="gmHomeCartBtn" disabled>${officialShopIcon('cart')} Ajouter au panier</button>`:`<button type="button" class="gmHomeGhostBtn" onclick="location.hash='#boutique/${encodeURIComponent(company.shopSlug||slugify(company.name||''))}';render()">Voir la boutique</button><button type="button" class="gmHomeCartBtn" onclick="addGlobalMarketItemToCart('${esc(i.companyId)}','${esc(i.id)}')">${officialShopIcon('cart')} Ajouter au panier</button>`;
  return `<article class="gmHomeCatalogCard" data-type="${type}" data-cat="${esc(category)}" data-search="${esc((String(i.name||'')+' '+category+' '+shop+' '+desc).toLowerCase())}" data-date="${dt}" data-price="${price}" data-name="${esc(String(i.name||'').toLowerCase())}">
...
function renderGlobalShop(){
  ...
  const displayItems=realItems.length?realItems:globalMarketDemoItems();
  ...
  const demoNotice=!realItems.length?'<div class="gmHomeDemoNotice">Aucun article public n’est encore publié : la grille ci-dessous présente des exemples visuels de la future vitrine.</div>':'';
  app.innerHTML=`<main class="gmHomePage">
    ...
    <section class="gmHomeCatalogSection">${demoNotice}<div class="gmHomeCatalogHead"><div><h2>Produits & services disponibles</h2><p>Découvrez les offres publiées par les boutiques et entreprises partenaires.</p></div><span id="gmHomeResultCount"></span></div><div id="gmHomeCatalogGrid" class="gmHomeCatalogGrid">${cards}</div><div id="gmHomeEmpty" class="gmHomeEmpty" hidden><b>Aucun résultat</b><span>Modifiez votre recherche ou vos filtres.</span></div><nav id="gmHomePagination" class="gmHomePagination" aria-label="Pagination du catalogue"></nav></section>
    <footer class="gmHomeFooter"><div>${globalMarketBrandMark()}<span><b>GLOBAL MARKET</b><small>Produits & services en toute confiance</small></span></div><p>Marketplace multi-entreprises · © 2026 MEGA SERVICES SARL U</p></footer>
  </main>`;
...
function renderPublicShop(slug){
  ...
  app.innerHTML=globalUniversalHeader('shop')+`<div class="publicShop officialStore">
    ...
    <footer class="officialFooter"><div><b>${esc(bannerName)}</b><span>${esc(companySlogan)}</span></div><p>© 2026 ${esc(bannerName)} — Boutique officielle propulsée par GLOBAL MARKET.</p></footer>
  </div>`;
