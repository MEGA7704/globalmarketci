...
for(const x of ['v605ForceCatalogReconcile','v6:catalog:last-good','catalog-v605','V6_SCHEMA_VERSION','v6MigrateLegacy','withSession','X-D1-Bookmark','handleV6Catalog','LIMIT ? OFFSET ?','handleV6ClientOrders','handleV6AdminOrders','REALTIME_HUB','GLOBAL_MARKET_MEDIA','handleV6AdminMarketplaceSnapshot'])if(!worker.includes(x))fail('Worker V6 incomplet : '+x);
for(const x of ['gmV6StableCatalogItems','GM_V6_LAST_GOOD_CATALOG_ITEMS','/api/v6/bootstrap','/api/v6/catalog','GM_V6_D1_BOOKMARK','new WebSocket','gmV6ConnectRealtime','gmV6RefreshAdminMarketplace'])if(!app.includes(x))fail('Client V6 incomplet : '+x);
...
console.log('[validate V6.0.5] OK — Pages déployable sans R2 ni Worker temps réel préexistants; D1 relationnel actif, R2 et WebSocket restent activables ensuite.');
