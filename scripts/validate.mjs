...
const required=['public/index.html','public/assets/app.js','public/assets/style.css','public/_worker.js','public/_routes.json','wrangler.json','cloudflare/schema-v6.sql','realtime-worker/src/index.js','realtime-worker/wrangler.json','scripts/migrate-v6.mjs','README_MIGRATION_V6.md'];
for(const f of required)if(!fs.existsSync(f))fail('Fichier obligatoire absent : '+f);
const app=fs.readFileSync('public/assets/app.js','utf8'),worker=fs.readFileSync('public/_worker.js','utf8'),schema=fs.readFileSync('cloudflare/schema-v6.sql','utf8'),rt=fs.readFileSync('realtime-worker/src/index.js','utf8'),wr=JSON.parse(fs.readFileSync('wrangler.json','utf8')),rtwr=JSON.parse(fs.readFileSync('realtime-worker/wrangler.json','utf8'));
...
if(!wr.durable_objects?.bindings?.some(x=>x.name==='REALTIME_HUB'&&x.script_name==='global-market-realtime'))fail('Binding Durable Object externe absent');
...
console.log('[validate V6] OK — D1 relationnel, index, pagination SQL, R2, Sessions API et WebSocket Durable Object présents.');
