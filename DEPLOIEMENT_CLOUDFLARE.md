...
4. Vérifier que le journal affiche `global-market-cloudflare@4.1.0 build`.
...
Aucune nouvelle ressource Cloudflare n’est nécessaire. Les tables D1 sont créées automatiquement par le Worker au premier appel API. Une sauvegarde/export des données actuelles reste recommandée avant le déploiement.

Après déploiement, vérifiez `/api/health`. Les valeurs attendues comprennent :

```json
{
  "storageVersion": 6,
  "storageMode": "normalized-records-v6"
}
```
